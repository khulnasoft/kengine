...
type Defaults struct {
	// Port we expect kengine to listening on
	AdminPort int
	// Certificates we expect to be loaded before attempting to run the tests
	Certificates []string
	// TestRequestTimeout is the time to wait for a http request to
	TestRequestTimeout time.Duration
	// LoadRequestTimeout is the time to wait for the config to be loaded against the kengine server
...
var Default = Defaults{
	...
	Certificates:       []string{"/kengine.localhost.crt", "/kengine.localhost.key"},
...
func (tc *Tester) initServer(rawConfig string, configType string) error {
	...
	rawConfig = prependKengineFilePath(rawConfig)
	...
	if err != nil {
		tc.t.Errorf("unable to contact kengine server. %s", err)
		return err
	}
	timeElapsed(start, "kenginetest: config load time")
...
func (tc *Tester) ensureConfigRunning(rawConfig string, configType string) error {
	expectedBytes := []byte(prependKengineFilePath(rawConfig))
	if configType != "json" {
		adapter := kengineconfig.GetAdapter(configType)
		if adapter == nil {
...
// designated path and Kengine sub-process is running.
func validateTestPrerequisites(t testing.TB) error {
	...
	for _, certName := range Default.Certificates {
		if _, err := os.Stat(getIntegrationDir() + certName); errors.Is(err, fs.ErrNotExist) {
			return fmt.Errorf("kengine integration test certificates (%s) not found", certName)
		}
	}

	if isKengineAdminRunning() != nil {
		...
		// start inprocess kengine server
		os.Args = []string{"kengine", "run", "--config", f.Name(), "--adapter", "kenginefile"}
		go func() {
			kenginecmd.Main()
		}()

		// wait for kengine to start serving the initial config
		for retries := 10; retries > 0 && isKengineAdminRunning() != nil; retries-- {
			time.Sleep(1 * time.Second)
		}
	}

	// one more time to return the error
	return isKengineAdminRunning()
}

func isKengineAdminRunning() error {
	// assert that kengine is running
	client := &http.Client{
		Timeout: Default.LoadRequestTimeout,
	}
	resp, err := client.Get(fmt.Sprintf("http://localhost:%d/config/", Default.AdminPort))
	if err != nil {
		return fmt.Errorf("kengine integration test kengine server not running. Expected to be listening on localhost:%d", Default.AdminPort)
	}
	resp.Body.Close()
...
// to run in any path
func prependKengineFilePath(rawConfig string) string {
...
func CreateTestingTransport() *http.Transport {
	...
	dialContext := func(ctx context.Context, network, addr string) (net.Conn, error) {
		...
		log.Printf("kenginetest: redirecting the dialer from %s to %s", addr, destAddr)
...
func AssertAdapt(t testing.TB, rawConfig string, adapterName string, expectedResponse string) {
	ok := CompareAdapt(t, "Kenginefile", rawConfig, adapterName, expectedResponse)
	if !ok {
		t.Fail()
	}
}

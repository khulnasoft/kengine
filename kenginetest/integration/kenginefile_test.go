...
func TestReadCookie(t *testing.T) {
  ...
  `, "kenginefile")
	...
	// act and assert
	tester.AssertGetResponse("http://localhost:9080/cookie.html", 200, "<h2>Cookie.ClientName kenginetest</h2>")
}
...
func TestInvalidPrefix(t *testing.T) {
	...
	for _, failureCase := range failureCases {
		kenginetest.AssertLoadError(t, failureCase.config, "kenginefile", failureCase.expectedError)
	}
}
...
func TestValidPrefix(t *testing.T) {
	...
	for _, successCase := range successCases {
		kenginetest.AssertAdapt(t, successCase.rawConfig, "kenginefile", successCase.expectedResponse)
	}
}
...
func TestHttpRequestLocalPortPlaceholder(t *testing.T) {
	...
	:9080
	respond "{http.request.local.port}"`, "kenginefile")
	...
	failureCases := []testCase{
		{
			...
			expectedError: `Kenginefile:2: parsed 'handle' as a site address, but it is a known directive; directives must appear in a site block`,
		},
		{
			config: `
			...
			expectedError: `Kenginefile:2: parsed 'reverse_proxy' as a site address, but it is a known directive; directives must appear in a site block`,
		},
	}

...
var (
	...
	// kenginefileLoaders is the list of all Kenginefile loaders
	// in registration order.
	kenginefileLoaders []kenginefileLoader
)
...
func DescribePlugins() string {
	...
	str += "\nKenginefile loaders:\n"
	for _, name := range pl["kenginefile_loaders"] {
		str += "  " + name + "\n"
	}
...
func ListPlugins() map[string][]string {
	...
	// kenginefile loaders in registration order
	for _, loader := range kenginefileLoaders {
		p["kenginefile_loaders"] = append(p["kenginefile_loaders"], loader.name)
	}
	if defaultKenginefileLoader.name != "" {
		p["kenginefile_loaders"] = append(p["kenginefile_loaders"], defaultKenginefileLoader.name)
	}
...
// between loading the Kenginefile, then executing its
// directives, then making the servers for Kengine to
// manage. Typically, such state involves configuration
// structs, etc.
type Context interface {
	// Called after the Kenginefile is parsed into server
	...
	// configuration file (Kenginefile).
	//
	// This function can be a no-op and simply return its
	// input if there is nothing to do here.
	InspectServerBlocks(string, []kenginefile.ServerBlock) ([]kenginefile.ServerBlock, error)

	// This is what Kengine calls to make server instances.
	// By this time, all directives have been executed and,
	// presumably, the context has enough state to produce
	// server instances for Kengine to start.
...
type ServerType struct {
	...
	// recommended, otherwise a blank Kenginefile will be
	// used.
	DefaultInput func() Input

	// The function that produces a new server type context.
	// This will be called when a new Kenginefile is being
...
type Plugin struct {
	...
	// with a directive in the Kenginefile.
	Action SetupFunc
}
...
// use 'go' keyword if they don't want to block Kengine.
...
// Loader is a type that can load a Kenginefile.
// It is passed the name of the server type.
// It returns an error only if something went
// wrong, not simply if there is no Kenginefile
// for this loader to load.
//
// A Loader should only load the Kenginefile if
...
// In other words, loading the Kenginefile must
// be deliberate & deterministic, not haphazard.
//
// The exception is the default Kenginefile loader,
// which will be called only if no other Kenginefile
...
// Load loads a Kenginefile.
func (lf LoaderFunc) Load(serverType string) (Input, error) {
	return lf(serverType)
}

// RegisterKenginefileLoader registers loader named name.
func RegisterKenginefileLoader(name string, loader Loader) {
	kenginefileLoaders = append(kenginefileLoaders, kenginefileLoader{name: name, loader: loader})
}

// SetDefaultKenginefileLoader registers loader by name
// as the default Kenginefile loader if no others produce
// a Kenginefile. If another Kenginefile loader has already
// been set as the default, this replaces it.
//
// Do not call RegisterKenginefileLoader on the same
// loader; that would be redundant.
func SetDefaultKenginefileLoader(name string, loader Loader) {
	defaultKenginefileLoader = kenginefileLoader{name: name, loader: loader}
}

// loadKenginefileInput iterates the registered Kenginefile loaders
// and, if needed, calls the default loader, to load a Kenginefile.
// It is an error if any of the loaders return an error or if
// more than one loader returns a Kenginefile.
func loadKenginefileInput(serverType string) (Input, error) {
	var loadedBy string
	var kenginefileToUse Input
	for _, l := range kenginefileLoaders {
		cdyfile, err := l.loader.Load(serverType)
		if err != nil {
			return nil, fmt.Errorf("loading Kenginefile via %s: %v", l.name, err)
		}
		if cdyfile != nil {
			if kenginefileToUse != nil {
				return nil, fmt.Errorf("Kenginefile loaded multiple times; first by %s, then by %s", loadedBy, l.name)
			}
			loaderUsed = l
			kenginefileToUse = cdyfile
			loadedBy = l.name
		}
	}
	if kenginefileToUse == nil && defaultKenginefileLoader.loader != nil {
		cdyfile, err := defaultKenginefileLoader.loader.Load(serverType)
		if err != nil {
			return nil, err
		}
		if cdyfile != nil {
			loaderUsed = defaultKenginefileLoader
			kenginefileToUse = cdyfile
		}
	}
	return kenginefileToUse, nil
}
...
// kenginefileLoader pairs the name of a loader to the loader.
type kenginefileLoader struct {
	name   string
	loader Loader
}

var (
	defaultKenginefileLoader kenginefileLoader // the default loader if all else fail
	loaderUsed             kenginefileLoader // the loader that was used (relevant for reloads)
)

...
import (
	...
	_ "github.com/khulnasoft/kengine/kenginehttp" // plug in the HTTP server type
...
func init() {
	...
	flag.StringVar(&conf, "conf", "", "Kenginefile to load (default \""+kengine.DefaultConfigFile+"\")")
	...
	flag.BoolVar(&fromJSON, "json-to-kenginefile", false, "From JSON stdin to Kenginefile stdout")
	...
	flag.StringVar(&kengine.PidFile, "pidfile", "", "Path to write pid file")
	flag.BoolVar(&kengine.Quiet, "quiet", false, "Quiet mode (no initialization output)")
	flag.StringVar(&revoke, "revoke", "", "Hostname for which to revoke the certificate")
	flag.StringVar(&serverType, "type", "http", "Type of server to run")
	flag.BoolVar(&toJSON, "kenginefile-to-json", false, "From Kenginefile stdin to JSON stdout")
	flag.BoolVar(&version, "version", false, "Show version")
	flag.BoolVar(&validate, "validate", false, "Parse the Kenginefile but do not start the server")

	kengine.RegisterKenginefileLoader("flag", kengine.LoaderFunc(confLoader))
	kengine.SetDefaultKenginefileLoader("default", kengine.LoaderFunc(defaultLoader))
}

// Run is Kengine's main() function.
func Run() {
	flag.Parse()
	...
	kengine.AppName = appName
	kengine.AppVersion = module.Version
	kengine.OnProcessExit = append(kengine.OnProcessExit, certmagic.CleanUpOwnLocks)
	...
	if plugins {
		fmt.Println(kengine.DescribePlugins())
		os.Exit(0)
	}

	// Check if we just need to do a Kenginefile Convert and exit
	checkJSONKenginefile()
	...
	kengine.EmitEvent(kengine.StartupEvent, nil)

	// Get Kenginefile input
	kenginefileinput, err := kengine.LoadKenginefile(serverType)
	if err != nil {
		mustLogFatalf("%v", err)
	}

	if validate {
		err := kengine.ValidateAndExecuteDirectives(kenginefileinput, nil, true)
		if err != nil {
			mustLogFatalf("%v", err)
		}
		msg := "Kenginefile is valid"
		fmt.Println(msg)
		log.Printf("[INFO] %s", msg)
		os.Exit(0)
	}

	// Log Kengine version before start
	log.Printf("[INFO] Kengine version: %s", module.Version)

	// Start your engines
	instance, err := kengine.Start(kenginefileinput)
	if err != nil {
		mustLogFatalf("%v", err)
	}

	// Begin telemetry (these are no-ops if telemetry disabled)
	telemetry.Set("kengine_version", module.Version)
...
func mustLogFatalf(format string, args ...interface{}) {
	if !kengine.IsUpgrade() {
		log.SetOutput(os.Stderr)
	}
	log.Fatalf(format, args...)
}

// confLoader loads the Kenginefile using the -conf flag.
func confLoader(serverType string) (kengine.Input, error) {
	if conf == "" {
		return nil, nil
	}

	if conf == "stdin" {
		return kengine.KenginefileFromPipe(os.Stdin, serverType)
	}

	var contents []byte
	if strings.Contains(conf, "*") {
		// Let kenginefile.doImport logic handle the globbed path
...
// defaultLoader loads the Kenginefile from the current working directory.
func defaultLoader(serverType string) (kengine.Input, error) {
	contents, err := ioutil.ReadFile(kengine.DefaultConfigFile)
	if err != nil {
	...
	return kengine.KenginefileInput{
		Contents:       contents,
		Filepath:       kengine.DefaultConfigFile,
		ServerTypeName: serverType,
	}, nil
}

// getBuildModule returns the build info of Kengine
...
func getBuildModule() *debug.Module {
	...
	if ok {
		// The recommended way to build Kengine involves
		// creating a separate main module, which
		// preserves kengine a read-only dependency
		// TODO: track related Go issue: https://github.com/golang/go/issues/29228
		for _, mod := range bi.Deps {
			if mod.Path == "github.com/khulnasoft/kengine" {
				return mod
			}
		}
	}
...
func checkJSONKenginefile() {
	if fromJSON {
		...
		kenginefileBytes, err := kenginefile.FromJSON(jsonBytes)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Converting from JSON failed: %v", err)
			os.Exit(2)
		}
		fmt.Println(string(kenginefileBytes))
		os.Exit(0)
	}
	if toJSON {
		kenginefileBytes, err := ioutil.ReadAll(os.Stdin)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Read stdin failed: %v", err)
			os.Exit(1)
		}
		jsonBytes, err := kenginefile.ToJSON(kenginefileBytes)
		if err != nil {
...
func initTelemetry() error {
	uuidFilename := filepath.Join(kengine.AssetsPath(), "uuid")
	if customUUIDFile := os.Getenv("KENGINE_UUID_FILE"); customUUIDFile != "" {
		uuidFilename = customUUIDFile
	}

	newUUID := func() uuid.UUID {
		id := uuid.New()
		err := os.MkdirAll(kengine.AssetsPath(), 0700)
		if err != nil {

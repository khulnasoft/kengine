...
// Config is the top (or beginning) of the Kengine configuration structure.
// Kengine config is expressed natively as a JSON document. If you prefer
// not to work with JSON directly, there are [many config adapters](/docs/config-adapters)
// available that can convert various inputs into Kengine JSON.
//
// Many parts of this config are extensible through the use of Kengine modules.
...
// which to use.
//
// Generally, all config settings are optional, as it is Kengine convention to
...
func changeConfig(method, path string, input []byte, ifMatchHeader string, forceReload bool) error {
	...
	// our old representation of kengine's actual config
	err = unsyncedDecodeAndRun(newCfg, true)
	if err != nil {
		if len(rawCfgJSON) > 0 {
			// restore old config state to keep it consistent
			// with what kengine is still running; we need to
	...
	// config to keep it consistent with what kengine is now
...
func provisionContext(newCfg *Config, replaceAdminServer bool) (Context, error) {
	...
	if replaceAdminServer {
		...
		if err != nil {
			return ctx, fmt.Errorf("starting kengine administration endpoint: %v", err)
		}
	}
...
// use to ensure a fully functional kengine instance.
...
func finishSettingUp(ctx Context, cfg *Config) error {
	...
	if cfg != nil && cfg.Admin != nil && cfg.Admin.Config != nil && cfg.Admin.Config.LoadRaw != nil {
		...
		logger := Log().Named("config_loader").With(
			zap.String("module", val.(Module).KengineModule().ID.Name()),
...
// ConfigLoader is a type that can load a Kengine config. If
// the return value is non-nil, it must be valid Kengine JSON;
...
// CustomVersion is an optional string that overrides Kengine's
// reported version. It can be helpful when downstream packagers
// need to manually set Kengine's version. If no other version
...
//
//	-ldflags '-X github.com/khulnasoft/kengine/v2.CustomVersion=v2.6.2'
//
// for example.
var CustomVersion string

// Version returns the Kengine version in a simple/short form, and
// a full version string. The short form will not have spaces and
// is intended for User-Agent strings and similar, but may be
// omitting valuable information. Note that Kengine must be compiled
...
// built Kengine from a git repository. If no version is available,
...
func Version() (simple, full string) {
	// the currently-recommended way to build Kengine involves
	...
	// find the Kengine module in the dependency list
...
// ImportPath is the package import path for Kengine core.
// This identifier may be removed in the future.
const ImportPath = "github.com/khulnasoft/kengine/v2"

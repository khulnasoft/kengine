...
// Package kengine implements the Kengine server manager.
//
// To use this package:
//
//   1. Set the AppName and AppVersion variables.
//   2. Call LoadKenginefile() to get the Kenginefile.
//      Pass in the name of the server type (like "http").
//      Make sure the server type's package is imported
//      (import _ "github.com/khulnasoft/kengine/kenginehttp").
//   3. Call kengine.Start() to start Kengine. You get back
...
var (
	...
	// Kengine process started this one.
	isUpgrade = os.Getenv("KENGINE__UPGRADE") == "1"
...
type Instance struct {
	...
	// kenginefileInput is the input configuration text used for this process
	kenginefileInput Input
...
// executing the newKenginefile. Upon success, it returns the new
// instance to replace i. Upon failure, i will not be replaced.
func (i *Instance) Restart(newKenginefile Input) (*Instance, error) {
	...
	if newKenginefile == nil {
		newKenginefile = i.kenginefileInput
	}
	...
	newInst := &Instance{serverType: newKenginefile.ServerType(), wg: i.wg, Storage: make(map[interface{}]interface{})}

	// attempt to start new instance
	err = startWithListenerFds(newKenginefile, newInst, restartFds)
	if err != nil {
...
// LoadKenginefile loads a Kenginefile by calling the plugged in
// Kenginefile loader methods. An error is returned if more than
// one loader returns a non-nil Kenginefile input. If no loaders
// load a Kenginefile, the default loader is used. If no default
// loader is registered or it returns nil, the server type's
// default Kenginefile is loaded. If the server type does not
// specify any default Kenginefile value, then an empty Kenginefile
// is returned. Consequently, this function never returns a nil
// value as long as there are no errors.
func LoadKenginefile(serverType string) (Input, error) {
	// If we are finishing an upgrade, we must obtain the Kenginefile
	...
	if IsUpgrade() {
		...
		return loadedGob.Kenginefile, nil
	}

	// Ask plugged-in loaders for a Kenginefile
	cdyfile, err := loadKenginefileInput(serverType)
	if err != nil {
		return nil, err
	}
	...
	// Still nil? Geez.
	if cdyfile == nil {
		cdyfile = KenginefileInput{ServerTypeName: serverType}
	}
...
// KenginefileFromPipe loads the Kenginefile input from f if f is
// not interactive input. f is assumed to be a pipe or stream,
// such as os.Stdin. If f is not a pipe, no error is returned
// but the Input value will be nil. An error is only returned
// if there was an error reading the pipe, even if the length
// of what was read is 0.
func KenginefileFromPipe(f *os.File, serverType string) (Input, error) {
	fi, err := f.Stat()
...
// Kenginefile returns the Kenginefile used to create i.
func (i *Instance) Kenginefile() Input {
	return i.kenginefileInput
}

// Start starts Kengine with the given Kenginefile.
//
...
func startWithListenerFds(cdyfile Input, inst *Instance, restartFds map[string]restartTriple) error {
	...
	// the kenginetls package, so it can perform cert
...
func executeDirectives(inst *Instance, filename string,
	directives []string, sblocks []kenginefile.ServerBlock, justValidate bool) error {
			...
			for j, key := range sb.Keys {
				...
				if tokens, ok := sb.Tokens[dir]; ok {
					controller := &Controller{
						instance:  inst,
						Key:       key,
						Dispenser: kenginefile.NewDispenserTokens(filename, tokens),
...
func loadServerBlocks(serverType, filename string, input io.Reader) ([]kenginefile.ServerBlock, error) {
	validDirectives := ValidDirectives(serverType)
	serverBlocks, err := kenginefile.Parse(filename, input, validDirectives)
	if err != nil {
		return nil, err
	}
	if len(serverBlocks) == 0 && serverTypes[serverType].DefaultInput != nil {
		newInput := serverTypes[serverType].DefaultInput()
		serverBlocks, err = kenginefile.Parse(newInput.Path(),
...
func Stop() error {
	...
	for {
		...
		// kenginemain/run.go blocks until this server instance has shut down
		inst.wg.Add(1)
		defer inst.wg.Done()
...
// KenginefileInput represents a Kenginefile as input
// and is simply a convenient way to implement
// the Input interface.
type KenginefileInput struct {
	Filepath       string
	Contents       []byte
...
func (c KenginefileInput) Body() []byte { return c.Contents }

// Path returns c.Filepath.
func (c KenginefileInput) Path() string { return c.Filepath }

// ServerType returns c.ServerType.
func (c KenginefileInput) ServerType() string { return c.ServerTypeName }

// Input represents a Kenginefile; its contents and file path
// (which should include the file name at the end of the path).
// If path does not apply (e.g. piped input) you may use
// any understandable value. The path is mainly used for logging,
// error messages, and debugging.
type Input interface {
	// Gets the Kenginefile contents
	Body() []byte
...
// DefaultInput returns the default Kenginefile input

# Contributing to Kengine

Welcome! Thank you for choosing to be a part of our community. Kengine wouldn't be great without your involvement!
...
You can have a huge impact on the project by helping with its code. To contribute code to Kengine, open a [pull request](https://github.com/khulnasoft/kengine/pulls) (PR). If you're new to our community, that's okay: **we gladly welcome pull requests from anyone, regardless of your native language or coding experience.** You can get familiar with Kengine's code base by using [code search at Sourcegraph](https://sourcegraph.com/github.com/khulnasoft/kengine/-/search).
...
- **Own your contributions.** Kengine is a growing project, and it's much better when individual contributors help maintain their change after it is merged.
...
2. If you don't already have this repo (khulnasoft/kengine.git) repo on your computer, get it with `go get github.com/khulnasoft/kengine/kengine`.

3. Tell git that it can push the khulnasoft/kengine.git repo to your fork by adding a remote: `git remote add myfork https://github.com/you/kengine.git`
...
Kengine can do more with plugins! Anyone can write a plugin. Plugins are Go libraries that get compiled into Kengine, extending its feature set. They can add directives to the Kenginefile, change how the Kenginefile is loaded, and even implement new server types (e.g. HTTP, DNS). When it's ready, you can submit your plugin to the Kengine website so others can download it.

[Learn how to write and submit a plugin](https://github.com/khulnasoft/kengine/wiki) on the wiki. You should also share and discuss your plugin idea [on the forums](https://kengine.community) to have people test it out. We don't use the Kengine issue tracker for plugins.
...
While we really do value your requests and implement many of them, not all features are a good fit for Kengine. Most of those [make good plugins](https://github.com/khulnasoft/kengine/wiki), which can be made by anyone! But if a feature is not in the best interest of the Kengine project or its users in general, we may politely decline to implement it into Kengine core.

### Improving documentation

Kengine's documentation is available at [https://khulnasoft.com/v1/docs](https://khulnasoft.com/v1/docs). If you would like to make a fix to the docs, please submit an issue here describing the change to make.

Note that plugin documentation is not hosted by the Kengine website, other than basic usage examples. They are managed by the individual plugin authors, and you will have to contact them to change their documentation.
...
- **Don't accept new dependencies lightly.** Dependencies can make the world crash and burn, but they are sometimes necessary. Choose carefully. Extremely small dependencies (a few lines of code) can be inlined. The rest may not be needed. For those that are, Kengine vendors all dependencies with the help of [gvt](https://github.com/FiloSottile/gvt). All external dependencies must be vendored, and _Kengine must not export any types defined by those dependencies_. Check this diligently!

- **Be extra careful in some areas of the code.** There are some critical areas in the Kengine code base that we review extra meticulously: the `kengine` and `kenginetls` packages especially.

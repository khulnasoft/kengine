...
func init() {
	kengine.RegisterModule(HTTPLoader{})
}

// HTTPLoader can load Kengine configs over HTTP(S).
//
...
func (HTTPLoader) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		ID:  "kengine.config_loaders.http",
		New: func() kengine.Module { return new(HTTPLoader) },
	}
}

// LoadConfig loads a Kengine config.
func (hl HTTPLoader) LoadConfig(ctx kengine.Context) ([]byte, error) {
...
func doHttpCallWithRetries(ctx kengine.Context, client *http.Client, request *http.Request) (*http.Response, error) {
...
func (hl HTTPLoader) makeClient(ctx kengine.Context) (*http.Client, error) {
	client := &http.Client{
...
var _ kengine.ConfigLoader = (*HTTPLoader)(nil)

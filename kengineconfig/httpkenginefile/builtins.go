...
func parseTLS(h Helper) ([]ConfigValue, error) {
	...
	cp := new(kenginetls.ConnectionPolicy)
	var fileLoader kenginetls.FileLoader
	var folderLoader kenginetls.FolderLoader
	var certSelector kenginetls.CustomCertSelectionPolicy
	var acmeIssuer *kenginetls.ACMEIssuer
	var keyType string
	var internalIssuer *kenginetls.InternalIssuer
	...
	case 1:
		if firstLine[0] == "internal" {
			internalIssuer = new(kenginetls.InternalIssuer)
	...
	case 2:
		...
		// https://github.com/khulnasoft/kengine/issues/2588 ... but we
		...
		if !ok {
			...
			fileLoader = append(fileLoader, kenginetls.CertKeyFilePair{
	...
	for h.NextBlock(0) {
		...
		case "protocols":
			...
			if len(args) > 0 {
				if _, ok := kenginetls.SupportedProtocols[args[0]]; !ok {
					return nil, h.Errf("wrong protocol name or protocol not supported: '%s'", args[0])
				}
				cp.ProtocolMin = args[0]
			}
			if len(args) > 1 {
				if _, ok := kenginetls.SupportedProtocols[args[1]]; !ok {
		...
		case "ciphers":
			for h.NextArg() {
				if !kenginetls.CipherSuiteNameSupported(h.Val()) {
		...
		case "curves":
			for h.NextArg() {
				if _, ok := kenginetls.SupportedCurves[h.Val()]; !ok {
		...
		case "client_auth":
			cp.ClientAuthentication = &kenginetls.ClientAuthentication{}
			if err := cp.ClientAuthentication.UnmarshalKenginefile(h.NewFromNextSegment()); err != nil {
				return nil, err
			}
		case "alpn":
			args := h.RemainingArgs()
		...
		case "ca":
			...
			if acmeIssuer == nil {
				acmeIssuer = new(kenginetls.ACMEIssuer)
			}
			acmeIssuer.CA = arg[0]
		...
		case "dns":
			...
			if acmeIssuer.Challenges == nil {
				acmeIssuer.Challenges = new(kenginetls.ChallengesConfig)
			}
			if acmeIssuer.Challenges.DNS == nil {
				acmeIssuer.Challenges.DNS = new(kenginetls.DNSChallengeConfig)
			}
			...
			acmeIssuer.Challenges.DNS.ProviderRaw = kengineconfig.JSONModuleObject(unm, "name", provName, h.warnings)
		...
		case "propagation_delay":
			...
			delayStr := arg[0]
			delay, err := kengine.ParseDuration(delayStr)
			if err != nil {
			...
			acmeIssuer.Challenges.DNS.PropagationDelay = kengine.Duration(delay)
		...
		case "propagation_timeout":
			...
			if timeoutStr == "-1" {
				...
				var err error
				timeout, err = kengine.ParseDuration(timeoutStr)
				if err != nil {
			...
			acmeIssuer.Challenges.DNS.PropagationTimeout = kengine.Duration(timeout)
		...
		case "dns_ttl":
			...
			ttlStr := arg[0]
			ttl, err := kengine.ParseDuration(ttlStr)
			if err != nil {
			...
			acmeIssuer.Challenges.DNS.TTL = kengine.Duration(ttl)
	...
	case acmeIssuer != nil:
		...
		defaultIssuers := kenginetls.DefaultIssuers(acmeIssuer.Email)
		...
		for _, issuer := range defaultIssuers {
			...
			if iss, ok := issuer.(*kenginetls.ACMEIssuer); ok && iss != nil {
				acmeCopy := *acmeIssuer
				acmeCopy.CA = iss.CA
				issuer = &acmeCopy
			}
...
func parseRoot(h Helper) ([]ConfigValue, error) {
	...
	if argsCount == 1 {
		...
		return h.NewRoute(nil, kenginehttp.VarsMiddleware{"root": h.Val()}), nil
	}
	...
	return h.NewRoute(userMatcherSet, kenginehttp.VarsMiddleware{"root": h.Val()}), nil
}

// parseFilesystem parses the fs directive. Syntax:
//
//	fs <filesystem>
func parseFilesystem(h Helper) (kenginehttp.MiddlewareHandler, error) {
	h.Next() // consume directive name
	if !h.NextArg() {
		return nil, h.ArgErr()
	}
	...
	return kenginehttp.VarsMiddleware{"fs": h.Val()}, nil
}

// parseVars parses the vars directive. See its UnmarshalKenginefile method for syntax.
func parseVars(h Helper) (kenginehttp.MiddlewareHandler, error) {
	v := new(kenginehttp.VarsMiddleware)
	err := v.UnmarshalKenginefile(h.Dispenser)
	return v, err
}
...
func parseRedir(h Helper) (kenginehttp.MiddlewareHandler, error) {
	h.Next() // consume directive name
	if !h.NextArg() {
		return nil, h.ArgErr()
	}
...
`
	...
	return kenginehttp.StaticResponse{
...
func parseRespond(h Helper) (kenginehttp.MiddlewareHandler, error) {
	sr := new(kenginehttp.StaticResponse)
	err := sr.UnmarshalKenginefile(h.Dispenser)
	return sr, err
}

// parseAbort parses the abort directive.
func parseAbort(h Helper) (kenginehttp.MiddlewareHandler, error) {
	h.Next() // consume directive
	for h.Next() || h.NextBlock(0) {
		return nil, h.ArgErr()
	}
	return &kenginehttp.StaticResponse{Abort: true}, nil
}

// parseError parses the error directive.
func parseError(h Helper) (kenginehttp.MiddlewareHandler, error) {
	se := new(kenginehttp.StaticError)
	err := se.UnmarshalKenginefile(h.Dispenser)
	return se, err
}

// parseRoute parses the route directive.
func parseRoute(h Helper) (kenginehttp.MiddlewareHandler, error) {
	...
	for _, result := range allResults {
		...
		case kenginehttp.Route, kenginehttp.Subroute:
		default:
...
func parseHandle(h Helper) (kenginehttp.MiddlewareHandler, error) {
...
func parseHandleErrors(h Helper) ([]ConfigValue, error) {
	...
	if expression != "" {
		statusMatcher := kengine.ModuleMap{
			"expression": h.JSON(kenginehttp.MatchExpression{Expr: expression}),
		}
		for i := range subroute.Routes {
			subroute.Routes[i].MatcherSetsRaw = []kengine.ModuleMap{statusMatcher}
		}
	}
	return []ConfigValue{
		{
...
func parseInvoke(h Helper) (kenginehttp.MiddlewareHandler, error) {
	h.Next() // consume directive
	if !h.NextArg() {
		return nil, h.ArgErr()
	}
	...
	// return the handler
	return &kenginehttp.Invoke{Name: h.Val()}, nil
}
...
func parseLogHelper(h Helper, globalLogNames map[string]struct{}) ([]ConfigValue, error) {
	...
	if parseAsGlobalOption {
		if h.NextArg() {
			...
			logName = kengine.DefaultLoggerName
		}
	...
	for h.NextBlock(0) {
		...
		case "output":
			...
			// can't use the usual kenginefile.Unmarshaler flow with the
			// standard writers because they are in the kengine package
			// (because they are the default) and implementing that
			// interface there would unfortunately create circular import
			var wo kengine.WriterOpener
			switch moduleName {
			case "stdout":
				wo = kengine.StdoutWriter{}
			case "stderr":
				wo = kengine.StderrWriter{}
			case "discard":
				wo = kengine.DiscardWriter{}
			default:
				modID := "kengine.logging.writers." + moduleName
				unm, err := kenginefile.UnmarshalModule(h.Dispenser, modID)
				if err != nil {
					return nil, err
				}
				var ok bool
				wo, ok = unm.(kengine.WriterOpener)
				if !ok {
					return nil, h.Errf("module %s (%T) is not a WriterOpener", modID, unm)
				}
			}
			cl.WriterRaw = kengineconfig.JSONModuleObject(wo, "output", moduleName, h.warnings)
		...
		case "core":
			...
			moduleName := h.Val()
			moduleID := "kengine.logging.cores." + moduleName
			unm, err := kenginefile.UnmarshalModule(h.Dispenser, moduleID)
			if err != nil {
				return nil, err
			}
			...
			cl.CoreRaw = kengineconfig.JSONModuleObject(core, "module", moduleName, h.warnings)
		...
		case "format":
			...
			cl.EncoderRaw = kengineconfig.JSONModuleObject(enc, "format", moduleName, h.warnings)
	...
	isEmptyConfig := reflect.DeepEqual(cl, new(kengine.CustomLog))
...
//
//	log_skip [<matcher>]
func parseLogSkip(h Helper) (kenginehttp.MiddlewareHandler, error) {
	h.Next() // consume directive name

	// "skip_log" is deprecated, replaced by "log_skip"
	if h.Val() == "skip_log" {
		kengine.Log().Named("config.adapter.kenginefile").Warn("the 'skip_log' directive is deprecated, please use 'log_skip' instead!")
	}

	if h.NextArg() {
		return nil, h.ArgErr()
	}
	return kenginehttp.VarsMiddleware{"log_skip": true}, nil
}

// parseLogName parses the log_name directive. Syntax:
//
//	log_name <names...>
func parseLogName(h Helper) (kenginehttp.MiddlewareHandler, error) {
	h.Next() // consume directive name
	return kenginehttp.VarsMiddleware{
		kenginehttp.AccessLoggerNameVarKey: h.RemainingArgs(),
	}, nil
}

...
// serverOptions collects server config overrides parsed from Kenginefile global options
type serverOptions struct {
	...
	// These will all map 1:1 to the kenginehttp.Server struct
	Name                 string
	ListenerWrappersRaw  []json.RawMessage
	ReadTimeout          kengine.Duration
	ReadHeaderTimeout    kengine.Duration
	WriteTimeout         kengine.Duration
	IdleTimeout          kengine.Duration
	KeepAliveInterval    kengine.Duration
	MaxHeaderBytes       int
...
func unmarshalKenginefileServerOptions(d *kenginefile.Dispenser) (any, error) {
	...
	for d.NextBlock(0) {
		...
		case "listener_wrappers":
			for nesting := d.Nesting(); d.NextBlock(nesting); {
				modID := "kengine.listeners." + d.Val()
				unm, err := kenginefile.UnmarshalModule(d, modID)
				if err != nil {
					return nil, err
				}
				listenerWrapper, ok := unm.(kengine.ListenerWrapper)
				if !ok {
		...
		case "timeouts":
			for nesting := d.Nesting(); d.NextBlock(nesting); {
				...
				case "read_body":
					...
					serverOpts.ReadTimeout = kengine.Duration(dur)
				...
				case "read_header":
					...
					serverOpts.ReadHeaderTimeout = kengine.Duration(dur)
				...
				case "write":
					...
					serverOpts.WriteTimeout = kengine.Duration(dur)
				...
				case "idle":
					...
					serverOpts.IdleTimeout = kengine.Duration(dur)
		...
		case "keepalive_interval":
			...
			serverOpts.KeepAliveInterval = kengine.Duration(dur)
		...
		case "trusted_proxies":
			...
			source, ok := unm.(kenginehttp.IPRangeSource)
			if !ok {
				return nil, fmt.Errorf("module %s (%T) is not an IP range source", modID, unm)
			}
			jsonSource := kengineconfig.JSONModuleObject(
				source,
				"source",
				source.(kengine.Module).KengineModule().ID.Name(),
				nil,
			)
		...
		case "metrics":
			...
			serverOpts.Metrics = new(kenginehttp.Metrics)
...
func applyServerOptions(
	...
	for key, server := range servers {
		...
		if opts.ShouldLogCredentials {
			if server.Logs == nil {
				server.Logs = new(kenginehttp.ServerLogConfig)
			}

...
import (
	...
	"github.com/khulnasoft/kengine/v2/modules/kenginepki"
...
// Kengine app module (e.g. third-party plugin) which was
...
// ServerType can set up a config from an HTTP Kenginefile.
type ServerType struct{}

// Setup makes a config from the tokens.
func (st ServerType) Setup(
	inputServerBlocks []kenginefile.ServerBlock,
	options map[string]any,
) (*kengine.Config, []kengineconfig.Warning, error) {
	...
	// with actual identifiers used by Kengine
	...
	for _, sb := range originalServerBlocks {
		...
		matcherDefs := make(map[string]kengine.ModuleMap)
		...
		for _, segment := range sb.block.Segments {
			...
			h := Helper{
				Dispenser:    kenginefile.NewDispenser(segment),
				options:      options,
				warnings:     &warnings,
			...
			if err != nil {
				return nil, warnings, fmt.Errorf("parsing kenginefile tokens for '%s': %v", dir, err)
			}
	...
	addCustomLog := func(ncl namedCustomLog) {
		...
		if ncl.name == kengine.DefaultLoggerName {
			hasDefaultLog = true
		}
	...
	if !hasDefaultLog {
		...
		if _, ok := options["debug"]; ok {
			customLogs = append(customLogs, namedCustomLog{
				name: kengine.DefaultLoggerName,
				log: &kengine.CustomLog{
	...
	cfg := &kengine.Config{AppsRaw: make(kengine.ModuleMap)}

	// loop through the configured options, and if any of
	// them are an httpkenginefile App, then we insert them
	// into the config as raw Kengine apps
	...
	// insert the standard Kengine apps into the config
	if len(httpApp.Servers) > 0 {
		cfg.AppsRaw["http"] = kengineconfig.JSON(httpApp, &warnings)
	}
	if !reflect.DeepEqual(tlsApp, &kenginetls.TLS{CertificatesRaw: make(kengine.ModuleMap)}) {
		cfg.AppsRaw["tls"] = kengineconfig.JSON(tlsApp, &warnings)
	}
	if !reflect.DeepEqual(pkiApp, &kenginepki.PKI{CAs: make(map[string]*kenginepki.CA)}) {
		cfg.AppsRaw["pki"] = kengineconfig.JSON(pkiApp, &warnings)
	}
	if filesystems, ok := options["filesystem"].(kengine.Module); ok {
		cfg.AppsRaw["kengine.filesystems"] = kengineconfig.JSON(
			filesystems,
			&warnings)
	}

	if storageCvtr, ok := options["storage"].(kengine.StorageConverter); ok {
		cfg.StorageRaw = kengineconfig.JSONModuleObject(storageCvtr,
			"module",
			storageCvtr.(kengine.Module).KengineModule().ID.Name(),
			&warnings)
	}
	if adminConfig, ok := options["admin"].(*kengine.AdminConfig); ok && adminConfig != nil {
		cfg.Admin = adminConfig
	}
	if pc, ok := options["persist_config"].(string); ok && pc == "off" {
		if cfg.Admin == nil {
			cfg.Admin = new(kengine.AdminConfig)
		}
		if cfg.Admin.Config == nil {
			cfg.Admin.Config = new(kengine.ConfigSettings)
		}
	...
	if len(customLogs) > 0 {
		if cfg.Logging == nil {
			cfg.Logging = &kengine.Logging{
				Logs: make(map[string]*kengine.CustomLog),
			}
		}
		...
		for _, ncl := range customLogs {
			if ncl.name == kengine.DefaultLoggerName && ncl.log != nil {
				cfg.Logging.Logs[kengine.DefaultLoggerName] = ncl.log
				break
			}
		}
		...
		for _, ncl := range customLogs {
			if ncl.log == nil || ncl.name == kengine.DefaultLoggerName {
				continue
			}
			if ncl.name != "" {
			...
			if ncl.name != kengine.DefaultLoggerName && len(ncl.log.Include) > 0 {
				defaultLog, ok := cfg.Logging.Logs[kengine.DefaultLoggerName]
				if !ok {
					defaultLog = new(kengine.CustomLog)
					cfg.Logging.Logs[kengine.DefaultLoggerName] = defaultLog
				}
...
func (ServerType) evaluateGlobalOptionsBlock(serverBlocks []serverBlock, options map[string]any) ([]serverBlock, error) {
	...
	for _, segment := range serverBlocks[0].block.Segments {
		...
		var val any
		var err error
		disp := kenginefile.NewDispenser(segment)
		...
		if err != nil {
			return nil, fmt.Errorf("parsing kenginefile tokens for '%s': %v", opt, err)
		}
...
func (ServerType) extractNamedRoutes(
	...
	options map[string]any,
	warnings *[]kengineconfig.Warning,
	replacer ShorthandReplacer,
) ([]serverBlock, error) {
	namedRoutes := map[string]*kenginehttp.Route{}
	...
	for _, sb := range serverBlocks {
		...
		wholeSegment := kenginefile.Segment{}
		for i := range sb.block.Segments {
		...
		h := Helper{
			Dispenser:    kenginefile.NewDispenser(wholeSegment),
			options:      options,
			warnings:     warnings,
			matcherDefs:  nil,
			parentBlock:  sb.block,
		...
		subroute := handler.(*kenginehttp.Subroute)
		...
		if len(subroute.Routes) == 1 && len(subroute.Routes[0].MatcherSetsRaw) == 0 {
			...
			route.HandlersRaw = []json.RawMessage{kengineconfig.JSONModuleObject(handler, "handler", subroute.KengineModule().ID.Name(), h.warnings)}
		}
...
func (st *ServerType) serversFromPairings(
...
) (map[string]*kenginehttp.Server, error) {
	servers := make(map[string]*kenginehttp.Server)
	...
	for i, p := range pairings {
		...
		srv := &kenginehttp.Server{
			Listen: p.addresses,
		}

		// handle the auto_https global option
		if autoHTTPS != "on" {
			srv.AutoHTTPS = new(kenginehttp.AutoHTTPSConfig)
			switch autoHTTPS {
			case "off":
		...
		// See https://github.com/khulnasoft/kengine/pull/4728 for a full explanation
		for _, sblock := range p.serverBlocks {
			for _, addr := range sblock.keys {
				if addr.Path != "" {
					kengine.Log().Named("kenginefile").Warn("Using a path in a site address is deprecated; please use the 'handle' directive instead", zap.String("address", addr.String()))
				}
			}
		}
		...
		sort.SliceStable(p.serverBlocks, func(i, j int) bool {
			...
			if iWildcardHost != jWildcardHost {
				...
				// https://github.com/khulnasoft/kengine/issues/3410
		...
		for _, sblock := range p.serverBlocks {
			if len(sblock.pile["custom_log"]) != 0 {
				srv.Logs = new(kenginehttp.ServerLogConfig)
				break
			}
		}

		// add named routes to the server if 'invoke' was used inside of it
		configuredNamedRoutes := options["named_routes"].(map[string]*kenginehttp.Route)
		for _, sblock := range p.serverBlocks {
			...
			for _, value := range sblock.pile[namedRouteKey] {
				if srv.NamedRoutes == nil {
					srv.NamedRoutes = map[string]*kenginehttp.Route{}
				}
		...
		for _, sblock := range p.serverBlocks {
			...
			for _, h := range hosts {
				if h == "0.0.0.0" || h == "::" {
					kengine.Log().Named("kenginefile").Warn("Site block has an unspecified IP address which only matches requests having that Host header; you probably want the 'bind' directive to configure the socket", zap.String("address", h))
				}
			}
			...
			if cpVals, ok := sblock.pile["tls.connection_policy"]; ok {
				...
				for _, cpVal := range cpVals {
					cp := cpVal.Value.(*kenginetls.ConnectionPolicy)
					...
					if len(hosts) > 0 {
						...
						cp.MatchersRaw = kengine.ModuleMap{
							"sni": kengineconfig.JSON(hosts, warnings), // make sure to match all hosts, not just auto-HTTPS-qualified ones
						}
					} else {
						cp.DefaultSNI = defaultSNI
						cp.FallbackSNI = fallbackSNI
			...
			for _, addr := range sblock.keys {
				...
				// https://kengine.community/t/making-sense-of-auto-https-and-why-disabling-it-still-serves-https-instead-of-http/9761
			...
			for _, listenerConfig := range sblock.pile["listener_wrapper"] {
				listenerWrapper, ok := listenerConfig.Value.(kengine.ListenerWrapper)
				if !ok {
					return nil, fmt.Errorf("config for a listener wrapper did not provide a value that implements kengine.ListenerWrapper")
				}
				jsonListenerWrapper := kengineconfig.JSONModuleObject(
					listenerWrapper,
					"wrapper",
					listenerWrapper.(kengine.Module).KengineModule().ID.Name(),
					warnings)
			...
			if errorSubrouteVals, ok := sblock.pile["error_route"]; ok {
				if srv.Errors == nil {
					srv.Errors = new(kenginehttp.HTTPErrorConfig)
				}
				sort.SliceStable(errorSubrouteVals, func(i, j int) bool {
					sri, srj := errorSubrouteVals[i].Value.(*kenginehttp.Subroute), errorSubrouteVals[j].Value.(*kenginehttp.Subroute)
					if len(sri.Routes[0].MatcherSetsRaw) == 0 && len(srj.Routes[0].MatcherSetsRaw) != 0 {
						return false
					}
					return true
				})
				errorsSubroute := &kenginehttp.Subroute{}
				for _, val := range errorSubrouteVals {
					sr := val.Value.(*kenginehttp.Subroute)
			...
			// see https://github.com/khulnasoft/kengine/issues/3310
				...
				if sblock.hasHostCatchAllKey() && len(ncl.hostnames) == 0 {
					...
					for _, h := range ncl.hostnames {
						if srv.Logs.LoggerNames == nil {
							srv.Logs.LoggerNames = make(map[string]kenginehttp.StringArray)
						}
		...
		// using the Kenginefile, user would expect all handshakes to at
		...
		// https://github.com/khulnasoft/kengine/issues/3004
		...
		if addressQualifiesForTLS &&
			...
			srv.TLSConnPolicies = append(srv.TLSConnPolicies, &kenginetls.ConnectionPolicy{DefaultSNI: defaultSNI, FallbackSNI: fallbackSNI})
		}
...
func detectConflictingSchemes(srv *kenginehttp.Server, serverBlocks []serverBlock, options map[string]any) error {
	httpPort := strconv.Itoa(kenginehttp.DefaultHTTPPort)
	if hp, ok := options["http_port"].(int); ok {
		httpPort = strconv.Itoa(hp)
	}
...
func consolidateConnPolicies(cps kenginetls.ConnectionPolicies) (kenginetls.ConnectionPolicies, error) {
...
func appendSubrouteToRouteList(routeList kenginehttp.RouteList,
	subroute *kenginehttp.Subroute,
	matcherSetsEnc []kengine.ModuleMap,
	p sbAddrAssociation,
	warnings *[]kengineconfig.Warning,
) kenginehttp.RouteList {
	...
	if wrapInSubroute {
		route := kenginehttp.Route{
			// the semantics of a site block in the Kenginefile dictate
		...
		if len(subroute.Routes) > 0 || subroute.Errors != nil {
			route.HandlersRaw = []json.RawMessage{
				kengineconfig.JSONModuleObject(subroute, "handler", "subroute", warnings),
			}
		}
...
func buildSubroute(routes []ConfigValue, groupCounter counter, needsSorting bool) (*kenginehttp.Subroute, error) {
	if needsSorting {
	...
	subroute := new(kenginehttp.Subroute)
	...
	mutuallyExclusiveDirs := map[string]*struct {
		...
		// (We use this on the Kengine website, or at least we did once.) The first rewrite's
	...
	for _, meDir := range keys {
		...
		// see khulnasoft/kengine#3108 - because the implied
	...
	for _, r := range routes {
		...
		if info, ok := mutuallyExclusiveDirs[r.directive]; ok {
			route := r.Value.(kenginehttp.Route)
			route.Group = info.groupName
			r.Value = route
		}

		switch route := r.Value.(type) {
		case kenginehttp.Subroute:
...
func normalizeDirectiveName(directive string) string {
	...
	// See https://github.com/khulnasoft/kengine/issues/3675#issuecomment-678042377
...
func consolidateRoutes(routes kenginehttp.RouteList) kenginehttp.RouteList {
...
func matcherSetFromMatcherToken(
	tkn kenginefile.Token,
	matcherDefs map[string]kengine.ModuleMap,
	warnings *[]kengineconfig.Warning,
) (kengine.ModuleMap, bool, error) {
	...
	if strings.HasPrefix(tkn.Text, "/") {
		return kengine.ModuleMap{
			"path": kengineconfig.JSON(kenginehttp.MatchPath{tkn.Text}, warnings),
		}, true, nil
	}
...
func (st *ServerType) compileEncodedMatcherSets(sblock serverBlock) ([]kengine.ModuleMap, error) {
	type hostPathPair struct {
		hostm kenginehttp.MatchHost
		pathm kenginehttp.MatchPath
	}
	...
	var matcherSets []map[string]kenginehttp.RequestMatcher
	for _, mp := range matcherPairs {
		matcherSet := make(map[string]kenginehttp.RequestMatcher)
		if len(mp.hostm) > 0 {
	...
	matcherSetsEnc := make([]kengine.ModuleMap, 0, len(matcherSets))
...
func parseMatcherDefinitions(d *kenginefile.Dispenser, matchers map[string]kengine.ModuleMap) error {
	...
	matchers[definitionName] = make(kengine.ModuleMap)

	// given a matcher name and the tokens following it, parse
	// the tokens as a matcher module and record it
	makeMatcher := func(matcherName string, tokens []kenginefile.Token) error {
		// create a new dispenser from the tokens
		dispenser := kenginefile.NewDispenser(tokens)
		...
		dispenser.SetContext(kenginefile.MatcherNameCtxKey, definitionName[1:])

		mod, err := kengine.GetModule("http.matchers." + matcherName)
		if err != nil {
			return fmt.Errorf("getting matcher module '%s': %v", matcherName, err)
		}
		unm, ok := mod.New().(kenginefile.Unmarshaler)
		if !ok {
			return fmt.Errorf("matcher module '%s' is not a Kenginefile unmarshaler", matcherName)
		}
		err = unm.UnmarshalKenginefile(dispenser)
		if err != nil {
			return err
		}
		rm, ok := unm.(kenginehttp.RequestMatcher)
		if !ok {
			return fmt.Errorf("matcher module '%s' is not a request matcher", matcherName)
		}
		matchers[definitionName][matcherName] = kengineconfig.JSON(rm, nil)
		return nil
	}
	...
	if d.NextArg() {
		if d.Token().Quoted() {
			...
			// see https://github.com/khulnasoft/kengine/issues/6287
			expressionToken := d.Token().Clone()
			expressionToken.Text = "expression"
			err := makeMatcher("expression", []kenginefile.Token{expressionToken, d.Token()})
			if err != nil {
				return err
			}
			return nil
	...
	// their tokens (we expect that UnmarshalKenginefile should be able to
	// handle more than one segment); otherwise, we'd overwrite other
	// instances of the matcher in this set
	tokensByMatcherName := make(map[string][]kenginefile.Token)
...
func encodeMatcherSet(matchers map[string]kenginehttp.RequestMatcher) (kengine.ModuleMap, error) {
	msEncoded := make(kengine.ModuleMap)
...
// likely a replaced shorthand of the known Kenginefile placeholder
...
func tryInt(val any, warnings *[]kengineconfig.Warning) int {
	intVal, ok := val.(int)
	if val != nil && !ok && warnings != nil {
		*warnings = append(*warnings, kengineconfig.Warning{Message: "not an integer type"})
	}
	return intVal
}

func tryString(val any, warnings *[]kengineconfig.Warning) string {
	stringVal, ok := val.(string)
	if val != nil && !ok && warnings != nil {
		*warnings = append(*warnings, kengineconfig.Warning{Message: "not a string type"})
	}
	return stringVal
}

func tryDuration(val any, warnings *[]kengineconfig.Warning) kengine.Duration {
	durationVal, ok := val.(kengine.Duration)
	if val != nil && !ok && warnings != nil {
		*warnings = append(*warnings, kengineconfig.Warning{Message: "not a duration type"})
	}
	return durationVal
}
...
// Mostly borrowed from unexported method in kenginehttp package.
func listenersUseAnyPortOtherThan(addresses []string, otherPort string) bool {
	...
	for _, lnAddr := range addresses {
		laddrs, err := kengine.ParseNetworkAddress(lnAddr)
		if err != nil {
			continue
		}
...
// Interface guard
var _ kenginefile.ServerType = (*ServerType)(nil)

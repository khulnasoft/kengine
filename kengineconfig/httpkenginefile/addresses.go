...
// isn't repeated unnecessarily. For example, this Kenginefile:
//
//	example.com {
//		bind 127.0.0.1
//	}
...
//	}
//
// has two server blocks to start with. But expressed in this Kenginefile are
...
func (st *ServerType) mapAddressToServerBlocks(originalServerBlocks []serverBlock,
	...
	for i, sblock := range originalServerBlocks {
		...
		addrToKeys := make(map[string][]kenginefile.Token)
...
// listenerAddrsForServerBlockKey essentially converts the Kenginefile
// site addresses to Kengine listener addresses for each server block.
func (st *ServerType) listenerAddrsForServerBlockKey(sblock serverBlock, key string,
	...
	httpPort, httpsPort := strconv.Itoa(kenginehttp.DefaultHTTPPort), strconv.Itoa(kenginehttp.DefaultHTTPSPort)
	...
	for _, lnHost := range lnHosts {
		...
		networkAddr := kengine.JoinNetworkAddress(network, host, lnPort)
		addr, err := kengine.ParseNetworkAddress(networkAddr)
		if err != nil {
...
func (a Address) String() string {
	...
	if a.Port != "" &&
		((scheme == "https" && a.Port != strconv.Itoa(kenginehttp.DefaultHTTPSPort)) ||
			(scheme == "http" && a.Port != strconv.Itoa(kenginehttp.DefaultHTTPPort))) {
...
// See https://github.com/khulnasoft/kengine/issues/3264

...
// consist of directives that are included in Kengine's
...
// is encountered in a Kenginefile, setupFunc will be called to
...
// to ('before' or 'after') a directive included in Kengine's
...
func RegisterDirectiveOrder(dir string, position Positional, standardDir string) {
	...
	if !foundStandardDir {
		panic("the 3rd argument '" + standardDir + "' must be a directive that exists in the standard distribution of Kengine")
	}
...
// option opt is encountered in a Kenginefile, setupFunc will be
...
// Kenginefile tokens.
type Helper struct {
	*kenginefile.Dispenser
	// State stores intermediate variables during kenginefile adaptation.
	State        map[string]any
	options      map[string]any
	warnings     *[]kengineconfig.Warning
	matcherDefs  map[string]kengine.ModuleMap
	parentBlock  kenginefile.ServerBlock
...
// Kenginefiles returns the list of config files from
// which tokens in the current server block were loaded.
func (h Helper) Kenginefiles() []string {
...
func (h Helper) JSON(val any) json.RawMessage {
	return kengineconfig.JSON(val, h.warnings)
}
...
func (h Helper) MatcherToken() (kengine.ModuleMap, bool, error) {
	if !h.NextArg() {
...
func (h Helper) ExtractMatcherSet() (kengine.ModuleMap, error) {
...
func (h Helper) NewRoute(matcherSet kengine.ModuleMap,
	handler kenginehttp.MiddlewareHandler,
) []ConfigValue {
	mod, err := kengine.GetModule(kengine.GetModuleID(handler))
	if err != nil {
		*h.warnings = append(*h.warnings, kengineconfig.Warning{
			File:    h.File(),
			Line:    h.Line(),
			Message: err.Error(),
		})
	...
	var matcherSetsRaw []kengine.ModuleMap
	if matcherSet != nil {
	...
	return []ConfigValue{
		{
			Class: "route",
			Value: kenginehttp.Route{
				MatcherSetsRaw: matcherSetsRaw,
				HandlersRaw:    []json.RawMessage{kengineconfig.JSONModuleObject(handler, "handler", mod.ID.Name(), h.warnings)},
			},
		},
	}
}

// GroupRoutes adds the routes (kenginehttp.Route type) in vals to the
// same group, if there is more than one route in vals.
func (h Helper) GroupRoutes(vals []ConfigValue) {
	// ensure there's at least two routes; group of one is pointless
	var count int
	for _, v := range vals {
		if _, ok := v.Value.(kenginehttp.Route); ok {
			count++
			if count > 1 {
				break
			}
	...
	for i := range vals {
		if route, ok := vals[i].Value.(kenginehttp.Route); ok {
...
func (h Helper) WithDispenser(d *kenginefile.Dispenser) Helper {
	h.Dispenser = d
	return h
}
...
// and returned.
func ParseSegmentAsSubroute(h Helper) (kenginehttp.MiddlewareHandler, error) {
...
func parseSegmentAsConfig(h Helper) ([]ConfigValue, error) {
	...
	for h.Next() {
		...
		var segments []kenginefile.Segment
		...
		matcherDefs := make(map[string]kengine.ModuleMap, len(h.matcherDefs))
		...
		for i := 0; i < len(segments); i++ {
			...
			if strings.HasPrefix(seg.Directive(), matcherPrefix) {
				...
				err := parseMatcherDefinitions(kenginefile.NewDispenser(seg), matcherDefs)
				if err != nil {
					return nil, err
				}
		...
		for _, seg := range segments {
			...
			if !ok {
				return nil, h.Errf("unrecognized directive: %s - are you sure your Kenginefile structure (nesting and braces) is correct?", dir)
			}

			subHelper := h
			subHelper.Dispenser = kenginefile.NewDispenser(seg)
			subHelper.matcherDefs = matcherDefs

			results, err := dirFunc(subHelper)
			if err != nil {
				return nil, h.Errf("parsing kenginefile tokens for '%s': %v", dir, err)
			}
...
func sortRoutes(routes []ConfigValue) {
	...
	sort.SliceStable(routes, func(i, j int) bool {
		...
		iRoute, ok := routes[i].Value.(kenginehttp.Route)
		if !ok {
			return false
		}
		jRoute, ok := routes[j].Value.(kenginehttp.Route)
		if !ok {
			return false
		}

		// decode the path matchers if there is just one matcher set
		var iPM, jPM kenginehttp.MatchPath
...
// serverBlock pairs a Kenginefile server block with
// a "pile" of config values, keyed by class name,
// as well as its parsed keys for convenience.
type serverBlock struct {
	block kenginefile.ServerBlock
...
func (sb serverBlock) hostsFromKeys(loggerMode bool) []string {
	...
	for _, addr := range sb.keys {
		...
		if loggerMode &&
			addr.Port != "" &&
			addr.Port != strconv.Itoa(kenginehttp.DefaultHTTPPort) &&
			addr.Port != strconv.Itoa(kenginehttp.DefaultHTTPSPort) {
...
type (
	// UnmarshalFunc is a function which can unmarshal Kenginefile
	...
	// with Kenginefile support is very common, this is a more
	// convenient way to add a handler to the chain since a lot
	// of the details common to HTTP handlers are taken care of
	// for you. These are passed to a call to
	// RegisterHandlerDirective.
	UnmarshalHandlerFunc func(h Helper) (kenginehttp.MiddlewareHandler, error)

	// UnmarshalGlobalFunc is a function which can unmarshal Kenginefile
	...
	UnmarshalGlobalFunc func(d *kenginefile.Dispenser, existingVal any) (any, error)
)

...
func parsePKIApp(d *kenginefile.Dispenser, existingVal any) (any, error) {
	d.Next() // consume app name

	pki := &kenginepki.PKI{
		CAs: make(map[string]*kenginepki.CA),
	}
	for d.NextBlock(0) {
		switch d.Val() {
		case "ca":
			pkiCa := new(kenginepki.CA)
			if d.NextArg() {
				pkiCa.ID = d.Val()
				if d.NextArg() {
					return nil, d.ArgErr()
			...
			if pkiCa.ID == "" {
				pkiCa.ID = kenginepki.DefaultCAID
			}
			...
			for nesting := d.Nesting(); d.NextBlock(nesting); {
				...
				case "intermediate_lifetime":
					...
					pkiCa.IntermediateLifetime = kengine.Duration(dur)

				case "root":
					if pkiCa.Root == nil {
						pkiCa.Root = new(kenginepki.KeyPair)
					}
					for nesting := d.Nesting(); d.NextBlock(nesting); {
						switch d.Val() {
						case "cert":
				...
				case "intermediate":
					if pkiCa.Intermediate == nil {
						pkiCa.Intermediate = new(kenginepki.KeyPair)
					}
					for nesting := d.Nesting(); d.NextBlock(nesting); {
						switch d.Val() {
						case "cert":
...
func (st ServerType) buildPKIApp(
...
) (*kenginepki.PKI, []kengineconfig.Warning, error) {
	...
	var pkiApp *kenginepki.PKI
	unwrappedPki, ok := options["pki"].(*kenginepki.PKI)
	if ok {
		pkiApp = unwrappedPki
	} else {
		pkiApp = &kenginepki.PKI{CAs: make(map[string]*kenginepki.CA)}
	}
	...
	for _, p := range pairings {
		for _, sblock := range p.serverBlocks {
			...
			for _, caCfgValue := range sblock.pile["pki.ca"] {
				ca := caCfgValue.Value.(*kenginepki.CA)
				if skipInstallTrust {
					ca.InstallTrust = &falseBool
				}

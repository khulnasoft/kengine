...
func (st ServerType) buildTLSApp(
...
) (*kenginetls.TLS, []kengineconfig.Warning, error) {
	tlsApp := &kenginetls.TLS{CertificatesRaw: make(kengine.ModuleMap)}
	var certLoaders []kenginetls.CertificateLoader
	...
	if catchAllAP != nil {
		if tlsApp.Automation == nil {
			tlsApp.Automation = new(kenginetls.AutomationConfig)
		}
		...
		for _, sblock := range p.serverBlocks {
			...
			if certManagerVals, ok := sblock.pile["tls.cert_manager"]; ok {
				for _, certManager := range certManagerVals {
					certGetterName := certManager.Value.(kengine.Module).KengineModule().ID.Name()
					ap.ManagersRaw = append(ap.ManagersRaw, kengineconfig.JSONModuleObject(certManager.Value, "via", certGetterName, &warnings))
				}
			}
			// custom bind host
			...
			// this is an example of a poor mapping from Kenginefile to JSON but that's
			...
			var ap2 *kenginetls.AutomationPolicy
			if len(ap.Issuers) == 0 {
				...
				if len(external) > 0 && len(internal) > 0 {
					...
					ap2.IssuersRaw = []json.RawMessage{kengineconfig.JSONModuleObject(kenginetls.InternalIssuer{}, "module", "internal", &warnings)}
				}
			}
			if tlsApp.Automation == nil {
				tlsApp.Automation = new(kenginetls.AutomationConfig)
			...
			if clVals, ok := sblock.pile["tls.cert_loader"]; ok {
				for _, clVal := range clVals {
					certLoaders = append(certLoaders, clVal.Value.(kenginetls.CertificateLoader))
				}
			}
		}
	}
	...
	if len(certLoaders) > 0 {
		loadersByName := make(map[string]kenginetls.CertificateLoader)
		for _, cl := range certLoaders {
			name := kengine.GetModuleName(cl)
			...
			if reflect.TypeOf(cl).Kind() == reflect.Slice {
				...
				loadersByName[name] = combined.Interface().(kenginetls.CertificateLoader)
			}
		}
		for certLoaderName, loaders := range loadersByName {
			tlsApp.CertificatesRaw[certLoaderName] = kengineconfig.JSON(loaders, &warnings)
		}
	}

	// set any of the on-demand options, for if/when on-demand TLS is enabled
	if onDemand, ok := options["on_demand_tls"].(*kenginetls.OnDemandConfig); ok {
		if tlsApp.Automation == nil {
			tlsApp.Automation = new(kenginetls.AutomationConfig)
		}
	...
	if storageCleanInterval, ok := options["storage_clean_interval"].(kengine.Duration); ok {
		if tlsApp.Automation == nil {
			tlsApp.Automation = new(kenginetls.AutomationConfig)
		}
	...
	if renewCheckInterval, ok := options["renew_interval"].(kengine.Duration); ok {
		if tlsApp.Automation == nil {
			tlsApp.Automation = new(kenginetls.AutomationConfig)
		}
	...
	if ocspCheckInterval, ok := options["ocsp_interval"].(kengine.Duration); ok {
		if tlsApp.Automation == nil {
			tlsApp.Automation = new(kenginetls.AutomationConfig)
		}
	...
	var al kenginetls.AutomateLoader
	internalAP := &kenginetls.AutomationPolicy{
	...
	if len(al) > 0 {
		tlsApp.CertificatesRaw["automate"] = kengineconfig.JSON(al, &warnings)
	}
		...
		if hasGlobalACMEDefaults {
			for i := 0; i < len(tlsApp.Automation.Policies); i++ {
				...
				if len(ap.Issuers) == 0 && automationPolicyHasAllPublicNames(ap) {
					...
					ap.Issuers = kenginetls.DefaultIssuers(emailStr)

					// if a specific endpoint is configured, can't use multiple default issuers
					if globalACMECA != nil {
						ap.Issuers = []certmagic.Issuer{new(kenginetls.ACMEIssuer)}
					}
				}
			}
		}
	...
	if tlsApp.Automation != nil {
		for i, ap := range tlsApp.Automation.Policies {
			...
			if len(ap.Issuers) > 0 && ap.IssuersRaw == nil {
				for _, iss := range ap.Issuers {
					issuerName := iss.(kengine.Module).KengineModule().ID.Name()
					ap.IssuersRaw = append(ap.IssuersRaw, kengineconfig.JSONModuleObject(iss, "module", issuerName, &warnings))
				}
			}
		}
		...
		if reflect.DeepEqual(tlsApp.Automation, new(kenginetls.AutomationConfig)) {
...
type acmeCapable interface{ GetACMEIssuer() *kenginetls.ACMEIssuer }
...
func fillInGlobalACMEDefaults(issuer certmagic.Issuer, options map[string]any) error {
	...
	if globalACMEDNS != nil && (acmeIssuer.Challenges == nil || acmeIssuer.Challenges.DNS == nil) {
		acmeIssuer.Challenges = &kenginetls.ChallengesConfig{
			DNS: &kenginetls.DNSChallengeConfig{
				ProviderRaw: kengineconfig.JSONModuleObject(globalACMEDNS, "name", globalACMEDNS.(kengine.Module).KengineModule().ID.Name(), nil),
			},
		}
	}
	...
	if globalPreferredChains != nil && acmeIssuer.PreferredChains == nil {
		acmeIssuer.PreferredChains = globalPreferredChains.(*kenginetls.ChainPreference)
	}
	if globalHTTPPort != nil && (acmeIssuer.Challenges == nil || acmeIssuer.Challenges.HTTP == nil || acmeIssuer.Challenges.HTTP.AlternatePort == 0) {
		...
		if acmeIssuer.Challenges.HTTP == nil {
			acmeIssuer.Challenges.HTTP = new(kenginetls.HTTPChallengeConfig)
		}
	...
	if globalHTTPSPort != nil && (acmeIssuer.Challenges == nil || acmeIssuer.Challenges.TLSALPN == nil || acmeIssuer.Challenges.TLSALPN.AlternatePort == 0) {
		...
		if acmeIssuer.Challenges.TLSALPN == nil {
			acmeIssuer.Challenges.TLSALPN = new(kenginetls.TLSALPNChallengeConfig)
		}
		acmeIssuer.Challenges.TLSALPN.AlternatePort = globalHTTPSPort.(int)
	}
	if globalCertLifetime != nil && acmeIssuer.CertificateLifetime == 0 {
		acmeIssuer.CertificateLifetime = globalCertLifetime.(kengine.Duration)
	}
	return nil
}
...
func newBaseAutomationPolicy(
	options map[string]any,
	_ []kengineconfig.Warning,
	always bool,
) (*kenginetls.AutomationPolicy, error) {
	...
	if !hasGlobalAutomationOpts {
		if always {
			return new(kenginetls.AutomationPolicy), nil
		}
		return nil, nil
	}

	ap := new(kenginetls.AutomationPolicy)
	if hasKeyType {
	...
	if hasIssuers {
		...
		ap.Issuers = []certmagic.Issuer{new(kenginetls.InternalIssuer)}
	}
...
func consolidateAutomationPolicies(aps []*kenginetls.AutomationPolicy) []*kenginetls.AutomationPolicy {
	...
	emptyAP := new(kenginetls.AutomationPolicy)
	for i := 0; i < len(aps); i++ {
...
// of b's subjects.
func automationPolicyIsSubset(a, b *kenginetls.AutomationPolicy) bool {
...
func automationPolicyShadows(i int, aps []*kenginetls.AutomationPolicy) int {
	for j := i + 1; j < len(aps); j++ {
		if automationPolicyIsSubset(aps[i], aps[j]) {
			return j
		}
...
func subjectQualifiesForPublicCert(ap *kenginetls.AutomationPolicy, subj string) bool {
...
func automationPolicyHasAllPublicNames(ap *kenginetls.AutomationPolicy) bool {

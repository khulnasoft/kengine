...
// Format formats the input Kenginefile to a standard, nice-looking
...
func Format(input []byte) []byte {
	...
	// the Kenginefile does not need any leading or trailing spaces, but...
	trimmedResult := bytes.TrimSpace(out.Bytes())

	// ...Kenginefiles should, however, end with a newline because

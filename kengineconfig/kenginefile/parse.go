...
func (p *parser) begin() error {
	...
	if p.eof {
		// this happens if the Kenginefile consists of only
	...
	if p.definedSnippets != nil && p.definedSnippets[importPattern] != nil {
		...
		if len(matches) == 0 {
			if strings.ContainsAny(globPattern, "*?[]") {
				kengine.Log().Warn("No files matching import glob pattern", zap.String("pattern", importPattern))
			} else {
...
func (p *parser) doSingleImport(importFile string) ([]Token, error) {
	...
	if len(input) == 0 || len(strings.TrimSpace(string(input))) == 0 {
		kengine.Log().Warn("Import file is empty", zap.String("file", importFile))
		return []Token{}, nil
	}

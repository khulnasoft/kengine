...
func parseVariadic(token Token, argCount int) (bool, int, int) {
	...
	if argRange == "" {
		kengine.Log().Named("kenginefile").Warn(
	...
	// https://github.com/khulnasoft/kengine/issues/5716
...
func makeArgsReplacer(args []string) *kengine.Replacer {
	repl := kengine.NewEmptyReplacer()

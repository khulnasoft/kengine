...
func RegisterAdapter(name string, adapter Adapter) {
	...
	kengine.RegisterModule(adapterModule{name, adapter})
}
...
// adapter into a Kengine module, which has the benefit of being
// counted with other modules, even though they do not
// technically extend the Kengine configuration structure.
// See khulnasoft/kengine#3132.
type adapterModule struct {
	name string
	Adapter
}

func (am adapterModule) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		ID:  kengine.ModuleID("kengine.adapters." + am.name),
		New: func() kengine.Module { return am },
	}
}

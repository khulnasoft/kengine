...
func TestSetup(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			c := kengine.NewTestController("http", test.input)
			c.Key = test.name
...
func TestCommandParse(t *testing.T) {
	tests := []struct {
		...
		{name: "noInput", input: `on`, shouldErr: true},
		{name: "nonExistent", input: "on xyz cmd arg", shouldErr: true},
		{name: "startup", input: `on startup cmd arg1 arg2`, shouldErr: false, config: hook.Config{Event: kengine.InstanceStartupEvent, Command: "cmd", Args: []string{"arg1", "arg2"}}},
		{name: "shutdown", input: `on shutdown cmd arg1 arg2 &`, shouldErr: false, config: hook.Config{Event: kengine.ShutdownEvent, Command: "cmd", Args: []string{"arg1", "arg2", "&"}}},
		{name: "certrenew", input: `on certrenew cmd arg1 arg2`, shouldErr: false, config: hook.Config{Event: kengine.CertRenewEvent, Command: "cmd", Args: []string{"arg1", "arg2"}}},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			config, err := onParse(kengine.NewTestController("http", test.input))

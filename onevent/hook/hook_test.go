...
func TestHook(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "blocking", event: kengine.InstanceStartupEvent, command: "mkdir", args: []string{osSensitiveTestDir}, shouldErr: false, shouldRemoveErr: false},
		{name: "nonBlocking", event: kengine.ShutdownEvent, command: "mkdir", args: []string{osSensitiveTestDir, "&"}, shouldErr: false, shouldRemoveErr: true},
		{name: "nonBlocking2", event: kengine.ShutdownEvent, command: "echo", args: []string{"&"}, shouldErr: false, shouldRemoveErr: true},
		{name: "nonExistent", event: kengine.CertRenewEvent, command: strconv.Itoa(int(time.Now().UnixNano())), shouldErr: true, shouldRemoveErr: true},
	}

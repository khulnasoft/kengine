...
func TestTemplateReload(t *testing.T) {
	...
	c := kengine.NewTestController("http", `markdown {
	...
	err = ioutil.WriteFile(targetFile, []byte("kengine"), 0644)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
	...
	if expect, got := "hello <p>kengine</p>\n", get(); expect != got {
		t.Fatalf("Expected body:\n%q\nbut got:\n%q", expect, got)
	}

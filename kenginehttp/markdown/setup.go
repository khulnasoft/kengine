...
func init() {
	kengine.RegisterPlugin("markdown", kengine.Plugin{
		ServerType: "http",
		Action:     setup,
	})
}
...
func markdownParse(c *kengine.Controller) ([]*Config, error) {
...
func loadParams(c *kengine.Controller, mdc *Config) error {
	cfg := httpserver.GetConfig(c)

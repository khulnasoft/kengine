...
func TestSetup(t *testing.T) {
	c := kengine.NewTestController("http", `gzip`)
	err := setup(c)
	if err != nil {
	...
	for i, test := range tests {
		_, err := gzipParse(kengine.NewTestController("http", test.input))
...
func TestShouldAddResponseFilters(t *testing.T) {
	configs, err := gzipParse(kengine.NewTestController("http", `gzip { min_length 654 }`))

...
func TestReverseProxyMaxConnLimit(t *testing.T) {
	...
	su, err := NewStaticUpstreams(kenginefile.NewDispenser("Testfile", strings.NewReader(`
		proxy / `+backend.URL+` {
...
func TestUnixSocketProxy(t *testing.T) {
	...
	dir, err := ioutil.TempDir("", "kengine_proxytest")
	if err != nil {
...
func testReverseProxyTransparentHeaders(t *testing.T, remoteAddr, forwardedForHeader string, expected []string) {
	...
	// make proxy
	upstreams, err := NewStaticUpstreams(kenginefile.NewDispenser("Testfile", strings.NewReader(config)), "")
	if err != nil {
...
func TestReverseProxyLargeBody(t *testing.T) {
	...
	su, err := NewStaticUpstreams(kenginefile.NewDispenser("Testfile", strings.NewReader(`proxy / `+backend.URL)), "")
	if err != nil {
		t.Fatal(err)
	}

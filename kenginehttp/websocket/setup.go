...
func init() {
	kengine.RegisterPlugin("websocket", kengine.Plugin{
		ServerType: "http",
		Action:     setup,
	})
}
...
func setup(c *kengine.Controller) error {
	...
	GatewayInterface = kengine.AppName + "-CGI/1.1"
	ServerSoftware = kengine.AppName + "/" + kengine.AppVersion
...
func webSocketParse(c *kengine.Controller) ([]Config, error) {
	var websocks []Config
	...
	for c.Next() {
		...
		cmd, args, err := kengine.SplitCommandAndArgs(command)
		if err != nil {
			return nil, err
		}

...
func TestMakePlaintextRedirects(t *testing.T) {
	configs := []*SiteConfig{
		...
		{Addr: Address{Host: "example.com"}, TLS: &kenginetls.Config{Managed: true}},

		// Host on port 80 already defined; don't change it (no redirect)
		{Addr: Address{Host: "sub1.example.com", Port: "80", Scheme: "http"}, TLS: new(kenginetls.Config)},
		{Addr: Address{Host: "sub1.example.com"}, TLS: &kenginetls.Config{Managed: true}},

		// Redirect from port 80 to port 5000 in this case
		{Addr: Address{Host: "sub2.example.com", Port: "5000"}, TLS: &kenginetls.Config{Managed: true}},

		// Can redirect from 80 to either 443 or 5001, but choose 443
		{Addr: Address{Host: "sub3.example.com", Port: "443"}, TLS: &kenginetls.Config{Managed: true}},
		{Addr: Address{Host: "sub3.example.com", Port: "5001", Scheme: "https"}, TLS: &kenginetls.Config{Managed: true}},
	}
...
func TestEnableAutoHTTPS(t *testing.T) {
	configs := []*SiteConfig{
		{Addr: Address{Host: "example.com"}, TLS: &kenginetls.Config{Managed: true, Manager: &certmagic.Config{}}},
...
func TestMarkQualifiedForAutoHTTPS(t *testing.T) {
	// TODO: kenginetls.TestQualifiesForManagedTLS and this test share nearly the same config list...
	configs := []*SiteConfig{
		...
		{Addr: Address{Host: "example.com"}, TLS: &kenginetls.Config{Manual: true}},
		{Addr: Address{Host: "example.com"}, TLS: &kenginetls.Config{ACMEEmail: "off"}},
		{Addr: Address{Host: "example.com"}, TLS: &kenginetls.Config{ACMEEmail: "foo@bar.com", Manager: &certmagic.Config{}}},
...
func newManagedConfig() *kenginetls.Config {
	return &kenginetls.Config{Manager: &certmagic.Config{}}
}

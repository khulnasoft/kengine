...
func (h *tlsHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// handshake (reported in kenginetls package) and our MITM assessment
	...
	go telemetry.AppendUnique("tls_client_hello_ua:"+kenginetls.ClientHelloInfo(info).Key(), uaHash)
...
func (c *clientHelloConn) Read(b []byte) (n int, err error) {
	...
	chKey := kenginetls.ClientHelloInfo(rawParsed).Key()
...
// https://jhalderm.com/pub/papers/interception-ndss17.pdf
type rawHelloInfo kenginetls.ClientHelloInfo

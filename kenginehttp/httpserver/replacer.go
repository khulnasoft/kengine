...
func (r *replacer) getSubstitution(key string) string {
	...
	case "{mitm}":
		if val, ok := r.request.Context().Value(kengine.CtxKey("mitm")).(bool); ok {
			if val {
				return "likely"
			}
			return "unlikely"
	...
	case "{tls_protocol}":
		if r.request.TLS != nil {
			if name, err := kenginetls.GetSupportedProtocolName(r.request.TLS.Version); err == nil {
				return name
			} else {
	...
	case "{tls_cipher}":
		if r.request.TLS != nil {
			if name, err := kenginetls.GetSupportedCipherName(r.request.TLS.CipherSuite); err == nil {
				return name
			} else {

...
import (
	...
	"github.com/khulnasoft/kengine/kenginefile"
	"github.com/khulnasoft/kengine/kenginehttp/staticfiles"
	"github.com/khulnasoft/kengine/kenginetls"
...
func init() {
	...
	kengine.RegisterKenginefileLoader("short", kengine.LoaderFunc(shortKenginefileLoader))
	kengine.RegisterParsingCallback(serverType, "root", hideKenginefile)
	kengine.RegisterParsingCallback(serverType, "tls", activateHTTPS)
	kenginetls.RegisterConfigGetter(serverType, func(c *kengine.Controller) *kenginetls.Config { return GetConfig(c).TLS })

	// disable the kenginetls package reporting ClientHellos
	// to telemetry, since our MITM detector does this but
	// with more information than the standard lib provides
	// (as of May 2018)
	kenginetls.ClientHelloTelemetry = false
}

// hideKenginefile hides the source/origin Kenginefile if it is within the
// site root. This function should be run after parsing the root directive.
func hideKenginefile(cctx kengine.Context) error {
	ctx := cctx.(*httpContext)
	for _, cfg := range ctx.siteConfigs {
		// if no Kenginefile exists exit.
		if cfg.originKenginefile == "" {
			return nil
		}
		...
		absOriginKenginefile, err := filepath.Abs(cfg.originKenginefile)
		if err != nil {
			return err
		}
		if strings.HasPrefix(absOriginKenginefile, absRoot) {
			cfg.HiddenFiles = append(cfg.HiddenFiles, filepath.ToSlash(strings.TrimPrefix(absOriginKenginefile, absRoot)))
		}
	}
	return nil
}

func newContext(inst *kengine.Instance) kengine.Context {
	return &httpContext{instance: inst, keysToSiteConfigs: make(map[string]*SiteConfig)}
}

type httpContext struct {
	instance *kengine.Instance
	...
	// that appeared in the Kenginefile.
...
func (h *httpContext) InspectServerBlocks(sourceFile string, serverBlocks []kenginefile.ServerBlock) ([]kenginefile.ServerBlock, error) {
	...
	for _, sb := range serverBlocks {
		for _, key := range sb.Keys {
			...
			// Make our kenginetls.Config, which has a pointer to the
			// instance's certificate cache and enough information
			// to use automatic HTTPS when the time comes
			kenginetlsConfig, err := kenginetls.NewConfig(h.instance)
			if err != nil {
				return nil, fmt.Errorf("creating new kenginetls configuration: %v", err)
			}
			kenginetlsConfig.Hostname = addr.Host
			kenginetlsConfig.Manager.AltHTTPPort = altHTTPPort
			kenginetlsConfig.Manager.AltTLSALPNPort = altTLSALPNPort

			// Save the config to our master list, and key it for lookups
			cfg := &SiteConfig{
				Addr:            addr,
				Root:            Root,
				TLS:             kenginetlsConfig,
				originKenginefile: sourceFile,
	...
	for _, sb := range serverBlocks {
		...
		if hasGzip && !hasErrors {
			sb.Tokens["errors"] = []kenginefile.Token{{Text: "errors"}}
		}
	}
...
func (h *httpContext) MakeServers() ([]kengine.Server, error) {
	httpPort := strconv.Itoa(certmagic.HTTPPort)
	httpsPort := strconv.Itoa(certmagic.HTTPSPort)
	...
	for _, publicCAEndpoint := range kenginetls.KnownACMECAs {
	...
	for _, cfg := range h.siteConfigs {
		...
		if !atLeastOneSiteLooksLikeProduction {
			if !kengine.IsLoopback(cfg.Addr.Host) &&
				!kengine.IsLoopback(cfg.ListenHost) &&
				(kenginetls.QualifiesForManagedTLS(cfg) ||
...
func GetConfig(c *kengine.Controller) *SiteConfig {
	...
	cfg := &SiteConfig{
		Root:       Root,
		TLS:        &kenginetls.Config{Manager: certmagic.NewDefault()},
		IndexPages: staticfiles.DefaultIndexPages,
	}
...
// shortKenginefileLoader loads a Kenginefile if positional arguments are
// detected, or, in other words, if un-named arguments are provided to
// the program. A "short Kenginefile" is one in which each argument
// is a line of the Kenginefile. The default host and port are prepended
// according to the Host and Port values.
func shortKenginefileLoader(serverType string) (kengine.Input, error) {
...
func groupSiteConfigsByListenAddr(configs []*SiteConfig) (map[string][]*SiteConfig, error) {
	...
	for _, conf := range configs {
		...
		// https://kengine.community/t/wildcard-virtual-domains-with-wildcard-roots/221/5?u=matt
...
var directives = []string{
	...
	"on",
	"supervisor", // github.com/lucaslorentz/kengine-supervisor
	"request_id",
	"realip", // github.com/captncraig/kengine-realip
	"git",    // github.com/abiosoft/kengine-git

	// directives that add listener middleware to the stack
	"proxyprotocol", // github.com/mastercactapus/kengine-proxyprotocol

	// directives that add middleware to the stack
	"locale", // github.com/simia-tech/kengine-locale
	"log",
	"cache", // github.com/nicolasazrak/kengine-cache
	"rewrite",
	"ext",
	"minify", // github.com/hacdias/kengine-minify
	"gzip",
	"header",
	"geoip", // github.com/kodnaplakal/kengine-geoip
	"errors",
	"authz",        // github.com/casbin/kengine-authz
	"filter",       // github.com/echocat/kengine-filter
	"ipfilter",     // github.com/pyed/ipfilter
	"ratelimit",    // github.com/xuqingfeng/kengine-rate-limit
	"recaptcha",    // github.com/defund/kengine-recaptcha
	"expires",      // github.com/epicagency/kengine-expires
	"forwardproxy", // github.com/khulnasoft/forwardproxy
	"basicauth",
	"redir",
	"status",
	"cors",      // github.com/captncraig/cors/kengine
	"s3browser", // github.com/techknowlogick/kengine-s3browser
	"nobots",    // github.com/Xumeiquer/nobots
	"mime",
	"login",      // github.com/tarent/loginsrv/kengine
	"reauth",     // github.com/freman/kengine-reauth
	"extauth",    // github.com/BTBurke/kengine-extauth
	"jwt",        // github.com/BTBurke/kengine-jwt
	"permission", // github.com/dhaavi/kengine-permission
	"jsonp",      // github.com/pschlump/kengine-jsonp
	"upload",     // blitznote.com/src/kengine.upload
	"multipass",  // github.com/namsral/multipass/kengine
	"internal",
	"pprof",
	"expvar",
	"push",
	"datadog",    // github.com/payintech/kengine-datadog
	"prometheus", // github.com/miekg/kengine-prometheus
	"templates",
	"proxy",
	"pubsub", // github.com/jung-kurt/kengine-pubsub
	"fastcgi",
	"cgi", // github.com/jung-kurt/kengine-cgi
	"websocket",
	"filebrowser", // github.com/filebrowser/kengine
	"webdav",      // github.com/hacdias/kengine-webdav
	"markdown",
	"browse",
	"mailout",   // github.com/SchumacherFM/mailout
	"awses",     // github.com/miquella/kengine-awses
	"awslambda", // github.com/coopernurse/kengine-awslambda
	"grpc",      // github.com/pieterlouw/kengine-grpc
	"gopkg",     // github.com/zikes/gopkg
	"restic",    // github.com/restic/kengine
	"wkd",       // github.com/emersion/kengine-wkd
	"dyndns",    // github.com/linkonoid/kengine-dyndns
}

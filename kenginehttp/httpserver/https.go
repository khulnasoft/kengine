...
func activateHTTPS(cctx kengine.Context) error {
	operatorPresent := !kengine.Started()

	if !kengine.Quiet && operatorPresent {
	...
	if !kengine.IsUpgrade() {
		...
		certCache, ok := ctx.instance.Storage[kenginetls.CertCacheInstStorageKey].(*certmagic.Cache)
...
func markQualifiedForAutoHTTPS(configs []*SiteConfig) {
	for _, cfg := range configs {
		if kenginetls.QualifiesForManagedTLS(cfg) && cfg.Addr.Scheme != "http" {
...
func enableAutoHTTPS(configs []*SiteConfig, loadCertificates bool) error {
	for _, cfg := range configs {
		...
		kenginetls.SetDefaultTLSParams(cfg.TLS)
...
func redirPlaintextHost(cfg *SiteConfig) *SiteConfig {
	...
	return &SiteConfig{
		...
		TLS:        &kenginetls.Config{Manager: cfg.TLS.Manager},

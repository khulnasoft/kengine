...
// Package httpserver implements an HTTP server on top of Kengine.
package httpserver
...
func makeTLSConfig(group []*SiteConfig) (*tls.Config, error) {
	...
	return kenginetls.MakeTLSConfig(tlsConfigs)
}
...
func (s *Server) Listen() (net.Listener, error) {
	...
	// Very important to return a concrete kengine.Listener
	// implementation for graceful restarts.
	return cln.(kengine.Listener), nil
}
...
func (s *Server) Serve(ln net.Listener) error {
	if s.Server.TLSConfig != nil {
		...
		s.tlsGovChan = kenginetls.RotateSessionTicketKeys(s.Server.TLSConfig)
	}
...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Server", kengine.AppName)
...
func (s *Server) serveHTTP(w http.ResponseWriter, r *http.Request) (int, error) {
	...
	c := context.WithValue(r.Context(), kengine.CtxKey("path_prefix"), pathPrefix)
	r = r.WithContext(c)
...
// and any relevant information, assuming kengine.Quiet == false.
...
func (s *Server) outputSiteInfo(isProcessLog bool) {
	for _, site := range s.sites {
		...
		if kengine.IsLoopback(s.Address()) && !kengine.IsLoopback(site.Addr.Host) {
...
func (ln tcpKeepAliveListener) Accept() (net.Conn, error) {
	...
	// https://github.com/khulnasoft/kengine/pull/2787
...
// File implements kengine.Listener; it returns the underlying file of the listener.
...
// requested host is not served by Kengine on a given port.
...
const OriginalURLCtxKey = kengine.CtxKey("original_url")

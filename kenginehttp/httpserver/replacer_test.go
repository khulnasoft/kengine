...
func TestReplace(t *testing.T) {
	...
	request.Header.Set("CustomAdd", "kengine")
	...
	testCases := []struct {
		...
		{"The CustomAdd header is {>CustomAdd}.", "The CustomAdd header is kengine."},
...
-----END CERTIFICATE-----`)
	...
	protocol, _ := kenginetls.GetSupportedProtocolName(request.TLS.Version)
	cipher, _ := kenginetls.GetSupportedCipherName(request.TLS.CipherSuite)
...
func TestSet(t *testing.T) {
	...
	repl.Set("host", "getkengine.com")
	repl.Set("method", "GET")
	repl.Set("status", "201")
	repl.Set("variable", "value")

	if repl.Replace("This host is {host}") != "This host is getkengine.com" {
...
func TestPathRewrite(t *testing.T) {
	...
	request, err := http.NewRequest("POST", "http://getkengine.com/index.php?key=value", reader)
	if err != nil {
		t.Fatalf("Request Formation Failed: %s\n", err.Error())
	}
	urlCopy := *request.URL

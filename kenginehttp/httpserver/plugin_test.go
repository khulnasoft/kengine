...
func TestInspectServerBlocksWithCustomDefaultPort(t *testing.T) {
	Port = "9999"
	filename := "Testfile"
	ctx := newContext(&kengine.Instance{Storage: make(map[interface{}]interface{})}).(*httpContext)
	input := strings.NewReader(`localhost`)
	sblocks, err := kenginefile.Parse(filename, input, nil)
	if err != nil {
...
func TestGetConfig(t *testing.T) {
	...
	con := kengine.NewTestController("http", "")
	con.Key = "foo"
	cfg := GetConfig(con)
	con.Key = "FOO"
	cfg2 := GetConfig(con)
...
// Test to make sure we are correctly hiding the Kenginefile
func TestHideKenginefile(t *testing.T) {
	ctx := newContext(&kengine.Instance{Storage: make(map[interface{}]interface{})}).(*httpContext)
	ctx.saveConfig("test", &SiteConfig{
		Root:            Root,
		originKenginefile: "Testfile",
	})
	err := hideKenginefile(ctx)
	if err != nil {
		t.Fatalf("Failed to hide Kenginefile, got: %v", err)
		return
	}
	if len(ctx.siteConfigs[0].HiddenFiles) == 0 {
		t.Fatal("Failed to add Kenginefile to HiddenFiles.")
		return
	}
	...
	t.Fatal("Kenginefile missing from HiddenFiles")
}

...
func TestInclude(t *testing.T) {
	...
	tests := []struct {
		...
		{
			fileContent:          `hello {{ kengine }}`,
			expectedContent:      "hello kengine",
			shouldErr:            false,
			expectedErrorContent: "",
		},
	}

	TemplateFuncs["kengine"] = func() string { return "kengine" }
...
func TestFiles(t *testing.T) {
	...
	for i, test := range tests {
		...
		if test.fileNames != nil {
			dirPath, err = ioutil.TempDir(fmt.Sprintf("%s", context.Root), "kengine_ctxtest")
			if err != nil {

...
func TestRedirect(t *testing.T) {
	for i, test := range []struct {
		...
		// help with redirects because of Kengine's design: a redirect middleware
		...
		re := Redirect{
			...
			Rules: []Rule{
				...
				// redirect rule with Kengine because http and https schemes have different listeners,
				// so they don't share a redirect rule. So although these tests prove something
				// impossible with Kengine, it's extra bulletproofing at very little cost.

...
func init() {
	// register KenginefileInput with gob so it knows into
	// which concrete type to decode an Input interface
	gob.Register(KenginefileInput{})
}

// IsUpgrade returns true if this process is part of an upgrade
// where a parent kengine process spawned this one to upgrade
// the binary.
func IsUpgrade() bool {
	mu.Lock()
	defer mu.Unlock()
...
func Upgrade() error {
	...
	// use existing Kenginefile; do not change configuration during upgrade
	currentKenginefile, _, err := getCurrentKenginefile()
	if err != nil {
		return err
	}
	...
	env := os.Environ()
	if !IsUpgrade() {
		env = append(env, "KENGINE__UPGRADE=1")
	}

	// prepare our payload to the child process
	cdyfileGob := transferGob{
		ListenerFds: make(map[string]uintptr),
		Kenginefile:   currentKenginefile,
	}

	// prepare a pipe to the fork's stdin so it can get the Kenginefile
...
// getCurrentKenginefile gets the Kenginefile used by the
// current (first) Instance and returns both of them.
func getCurrentKenginefile() (Input, *Instance, error) {
	instancesMu.Lock()
	if len(instances) == 0 {
		instancesMu.Unlock()
	...
	currentKenginefile := inst.kenginefileInput
	if currentKenginefile == nil {
		// hmm, did spawning process forget to close stdin? Anyhow, this is unusual.
		return nil, inst, fmt.Errorf("no Kenginefile to reload (was stdin left open?)")
	}
	return currentKenginefile, inst, nil
}
...
// restart, then try loading first host of Kenginefile in the browser
...
// the Kenginefile contents and any other state needed by the new process.

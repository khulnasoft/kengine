...
/*
// TODO
func TestKengineStartStop(t *testing.T) {
	kenginefile := "localhost:1984"

	for i := 0; i < 2; i++ {
		_, err := Start(KenginefileInput{Contents: []byte(kenginefile)})
		if err != nil {
...
func (h *CallbackTestContext) InspectServerBlocks(name string, sblock []kenginefile.ServerBlock) ([]kenginefile.ServerBlock, error) {
	return sblock, nil
}
...
func TestKengineRestartCallbacks(t *testing.T) {
	for i, test := range []struct {
		...
		_, err := c.instance.Restart(KenginefileInput{Contents: []byte(""), ServerTypeName: serverName})
		if err != nil {

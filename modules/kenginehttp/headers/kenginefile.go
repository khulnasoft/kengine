...
func init() {
	httpkenginefile.RegisterDirective("header", parseKenginefile)
	httpkenginefile.RegisterDirective("request_header", parseReqHdrKenginefile)
}

// parseKenginefile sets up the handler for response headers from
// Kenginefile tokens. Syntax:
//
...
func parseKenginefile(h httpkenginefile.Helper) ([]httpkenginefile.ConfigValue, error) {
	...
	for h.NextBlock(0) {
		...
		// https://kengine.community/t/v2-reverse-proxy-please-add-cors-example-to-the-docs/7349/19
	...
	var configValues []httpkenginefile.ConfigValue
...
// parseReqHdrKenginefile sets up the handler for request headers
// from Kenginefile tokens. Syntax:
//
//	request_header [<matcher>] [[+|-]<field> [<value|regexp>] [<replacement>]]
func parseReqHdrKenginefile(h httpkenginefile.Helper) ([]httpkenginefile.ConfigValue, error) {
	h.Next() // consume directive name
	matcherSet, err := h.ExtractMatcherSet()
	if err != nil {
		return nil, err
	...
	configValues := []httpkenginefile.ConfigValue{}
	...
	if err := KenginefileHeaderOp(hdr.Request, field, value, replacement); err != nil {
		return nil, h.Err(err.Error())
	}
...
// KenginefileHeaderOp applies a new header operation according to
...
func KenginefileHeaderOp(ops *HeaderOps, field, value string, replacement *string) error {
...
func applyHeaderOp(ops *HeaderOps, respHeaderOps *RespHeaderOps, field, value string, replacement *string) error {
	...
	case strings.HasPrefix(field, "?"): // default (conditional on not existing) - response headers only
		...
		if respHeaderOps.Require == nil {
			respHeaderOps.Require = &kenginehttp.ResponseMatcher{

...
func (app *App) Provision(ctx kengine.Context) error {
	...
	app.tlsApp = tlsAppIface.(*kenginetls.TLS)
	app.ctx = ctx
	...
	for srvName, srv := range app.Servers {
		...
		srv.events = eventsAppIface.(*kengineevents.App)
		srv.ctx = ctx
		...
		if srv.ListenerWrappersRaw != nil {
			...
			for i, val := range vals.([]any) {
				...
				srv.listenerWrappers = append(srv.listenerWrappers, val.(kengine.ListenerWrapper))
			}
			...
			if !hasTLSPlaceholder && len(srv.listenerWrappers) > 0 {
				srv.listenerWrappers = append([]kengine.ListenerWrapper{new(tlsPlaceholderWrapper)}, srv.listenerWrappers...)
			}
		}
...
func (app *App) Validate() error {
	...
	for srvName, srv := range app.Servers {
		...
		for _, addr := range srv.Listen {
			listenAddr, err := kengine.ParseNetworkAddress(addr)
			if err != nil {
			...
			for i := uint(0); i < listenAddr.PortRangeSize(); i++ {
				addr := kengine.JoinNetworkAddress(listenAddr.Network, listenAddr.Host, strconv.Itoa(int(listenAddr.StartPort+i)))
...
func (app *App) Start() error {
	...
	for srvName, srv := range app.Servers {
		...
		for _, lnAddr := range srv.Listen {
			listenAddr, err := kengine.ParseNetworkAddress(lnAddr)
			if err != nil {
...
func (app *App) Stop() error {
		...
		for _, server := range app.Servers {
			for _, na := range server.addresses {
				for _, addr := range na.Expand() {
					if kengine.ListenerUsage(addr.Network, addr.JoinHostPort(0)) < 2 {
	...
	stopH3Server := func(server *Server) {
		...
		// listener first and these listeners maybe unregistered thus won't be closed. kengine
...
const defaultIdleTimeout = kengine.Duration(5 * time.Minute)

// Interface guards
var (
	_ kengine.App         = (*App)(nil)
	_ kengine.Provisioner = (*App)(nil)
	_ kengine.Validator   = (*App)(nil)
)

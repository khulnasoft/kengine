...
func init() {
	kengine.RegisterModule(LogAppend{})
}
...
func (LogAppend) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		...
		New: func() kengine.Module { return new(LogAppend) },
	}
}
...
func (h LogAppend) ServeHTTP(w http.ResponseWriter, r *http.Request, next kenginehttp.Handler) error {
	...
	vars := ctx.Value(kenginehttp.VarsCtxKey).(map[string]any)
	repl := ctx.Value(kengine.ReplacerCtxKey).(*kengine.Replacer)
	extra := ctx.Value(kenginehttp.ExtraLogFieldsCtxKey).(*kenginehttp.ExtraLogFields)
...
// Interface guards
var (
	_ kenginehttp.MiddlewareHandler = (*LogAppend)(nil)
)

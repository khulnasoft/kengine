...
func init() {
	httpkenginefile.RegisterHandlerDirective("push", parseKenginefile)
}

// parseKenginefile sets up the push handler. Syntax:
//
	...
	for h.NextBlock(0) {
		...
		case "headers":
			...
			for nesting := h.Nesting(); h.NextBlock(nesting); {
				...
				switch len(args) {
				case 1:
					err = headers.KenginefileHeaderOp(&handler.Headers.HeaderOps, args[0], "", nil)
				case 2:
					err = headers.KenginefileHeaderOp(&handler.Headers.HeaderOps, args[0], args[1], nil)
				case 3:
					err = headers.KenginefileHeaderOp(&handler.Headers.HeaderOps, args[0], args[1], &args[2])
				default:
					return nil, h.ArgErr()
				}

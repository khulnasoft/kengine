	...
	for _, resource := range h.Resources {
		if c := h.logger.Check(zapcore.DebugLevel, "pushing resource"); c != nil {
			c.Write(
				...
				zap.Object("push_headers", kenginehttp.LoggableHTTPHeader{
					Header:               hdr,
...
func (h Handler) initializePushHeaders(r *http.Request, repl *kengine.Replacer) http.Header {
...
func (lp linkPusher) WriteHeader(statusCode int) {
	if links, ok := lp.ResponseWriter.Header()["Link"]; ok {
		...
		if val := kenginehttp.GetVar(lp.request.Context(), pushedLink); val == nil {
			if c := lp.handler.logger.Check(zapcore.DebugLevel, "pushing Link resources"); c != nil {
				c.Write(zap.Strings("linked", links))
			}
			kenginehttp.SetVar(lp.request.Context(), pushedLink, true)

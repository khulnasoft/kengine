...
func (a *ActiveHealthChecks) Provision(ctx kengine.Context, h *Handler) error {
	if !a.IsEnabled() {
		return nil
	}
	...
	if a.Interval == 0 {
		a.Interval = kengine.Duration(30 * time.Second)
	}
...
func (h *Handler) doActiveHealthCheckForAllHosts() {
	for _, upstream := range h.Upstreams {
		go func(upstream *Upstream) {
			...
			networkAddr, err := kengine.NewReplacer().ReplaceOrErr(upstream.Dial, true, true)
			if err != nil {
...
func (h *Handler) doActiveHealthCheck(dialInfo DialInfo, hostAddr string, networkAddr string, upstream *Upstream) error {
	...
	ctx := h.ctx.Context
	ctx = context.WithValue(ctx, kengine.ReplacerCtxKey, kengine.NewReplacer())
	ctx = context.WithValue(ctx, kenginehttp.VarsCtxKey, map[string]any{
	...
	ctx = context.WithValue(ctx, kenginehttp.OriginalRequestCtxKey, *req)
	...
	if h.HealthChecks.Active.ExpectStatus > 0 {
		if !kenginehttp.StatusCodeMatches(resp.StatusCode, h.HealthChecks.Active.ExpectStatus) {

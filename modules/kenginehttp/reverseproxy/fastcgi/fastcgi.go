...
func init() {
	kengine.RegisterModule(Transport{})
}
...
func (Transport) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		ID:  "http.reverse_proxy.transport.fastcgi",
		New: func() kengine.Module { return new(Transport) },
	}
}

// Provision sets up t.
func (t *Transport) Provision(ctx kengine.Context) error {
	t.logger = ctx.Logger()
	...
	t.serverSoftware = "Kengine/" + version

	// Set a relatively short default dial timeout.
	// This is helpful to make load-balancer retries more speedy.
	if t.DialTimeout == 0 {
		t.DialTimeout = kengine.Duration(3 * time.Second)
	}
...
func (t Transport) RoundTrip(r *http.Request) (*http.Response, error) {
	server := r.Context().Value(kenginehttp.ServerCtxKey).(*kenginehttp.Server)
	...
	if strings.Contains(r.URL.Path, "\x00") {
		return nil, kenginehttp.Error(http.StatusBadRequest, fmt.Errorf("invalid request path"))
	}
	...
	loggableReq := kenginehttp.LoggableHTTPRequest{
		Request:              r,
...
func (t Transport) buildEnv(r *http.Request) (envVars, error) {
	...
	// See https://github.com/khulnasoft/kengine/issues/3718
	if pathInfo == "" {
		pathInfo, _ = repl.GetString("http.matchers.file.remainder")
	}

	// SCRIPT_FILENAME is the absolute path of SCRIPT_NAME
	scriptFilename := kenginehttp.SanitizedPathJoin(root, scriptName)
	...
	if env["PATH_INFO"] != "" {
		env["PATH_TRANSLATED"] = kenginehttp.SanitizedPathJoin(root, pathInfo) // Info: http://www.oreilly.com/openbook/cgi/ch02_04.html
	}
	...
	if r.TLS != nil {
		...
		for _, cs := range kenginetls.SupportedCipherSuites() {
...
// Note that these are slightly different from SupportedProtocols in kenginetls/config.go

...
func init() {
	httpkenginefile.RegisterDirective("php_fastcgi", parsePHPFastCGI)
}
...
//	    capture_stderr
//	}
func (t *Transport) UnmarshalKenginefile(d *kenginefile.Dispenser) error {
	...
	for d.NextBlock(0) {
		...
		case "dial_timeout":
			...
			t.DialTimeout = kengine.Duration(dur)
		...
		case "read_timeout":
			...
			t.ReadTimeout = kengine.Duration(dur)
		...
		case "write_timeout":
			...
			t.WriteTimeout = kengine.Duration(dur)
...
func parsePHPFastCGI(h httpkenginefile.Helper) ([]httpkenginefile.ConfigValue, error) {
	if !h.Next() {
		return nil, h.ArgErr()
	}
	...
	for dispenser.Next() {
		for dispenser.NextBlock(0) {
			...
			case "dial_timeout":
				...
				dur, err := kengine.ParseDuration(dispenser.Val())
				if err != nil {
					return nil, dispenser.Errf("bad timeout value %s: %v", dispenser.Val(), err)
				}
				fcgiTransport.DialTimeout = kengine.Duration(dur)
				dispenser.DeleteN(2)
			...
			case "read_timeout":
				...
				fcgiTransport.ReadTimeout = kengine.Duration(dur)
				dispenser.DeleteN(2)
			...
			case "write_timeout":
				...
				fcgiTransport.WriteTimeout = kengine.Duration(dur)
				dispenser.DeleteN(2)
	...
	if indexFile != "off" {
		...
		redirMatcherSet := kengine.ModuleMap{
			"file": h.JSON(fileserver.MatchFile{
				TryFiles: []string{"{http.request.uri.path}/" + indexFile},
			}),
			"not": h.JSON(kenginehttp.MatchNot{
				MatcherSetsRaw: []kengine.ModuleMap{
					{
						"path": h.JSON(kenginehttp.MatchPath{"*/"}),
					},
				},
			}),
		}
		redirHandler := kenginehttp.StaticResponse{
			StatusCode: kenginehttp.WeakString(strconv.Itoa(http.StatusPermanentRedirect)),
			Headers:    http.Header{"Location": []string{"{http.request.orig_uri.path}/"}},
		}
		redirRoute := kenginehttp.Route{
			MatcherSetsRaw: []kengine.ModuleMap{redirMatcherSet},
			HandlersRaw:    []json.RawMessage{kengineconfig.JSONModuleObject(redirHandler, "handler", "static_response", nil)},
		}
		...
		rewriteMatcherSet := kengine.ModuleMap{
			"file": h.JSON(fileserver.MatchFile{
				TryFiles:  tryFiles,
		...
		rewriteRoute := kenginehttp.Route{
			MatcherSetsRaw: []kengine.ModuleMap{rewriteMatcherSet},
			HandlersRaw:    []json.RawMessage{kengineconfig.JSONModuleObject(rewriteHandler, "handler", "rewrite", nil)},
		}
	...
	rpMatcherSet := kengine.ModuleMap{
		"path": h.JSON(pathList),
	}

	// create the reverse proxy handler which uses our FastCGI transport
	rpHandler := &reverseproxy.Handler{
		TransportRaw: kengineconfig.JSONModuleObject(fcgiTransport, "protocol", "fastcgi", nil),
	}
	...
	rpRoute := kenginehttp.Route{
		MatcherSetsRaw: []kengine.ModuleMap{rpMatcherSet},
		HandlersRaw:    []json.RawMessage{kengineconfig.JSONModuleObject(rpHandler, "handler", "reverse_proxy", nil)},
	}
		...
		return []httpkenginefile.ConfigValue{
			{
				...
				Value: kenginehttp.Route{
					MatcherSetsRaw: []kengine.ModuleMap{userMatcherSet},
					HandlersRaw:    []json.RawMessage{kengineconfig.JSONModuleObject(subroute, "handler", "subroute", nil)},
				},
			},
		}, nil
	}

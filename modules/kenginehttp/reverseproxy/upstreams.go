...
func init() {
	kengine.RegisterModule(SRVUpstreams{})
	kengine.RegisterModule(AUpstreams{})
	kengine.RegisterModule(MultiUpstreams{})
}
...
type SRVUpstreams struct {
	...
	Refresh kengine.Duration `json:"refresh,omitempty"`
...
func (SRVUpstreams) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		ID:  "http.reverse_proxy.upstreams.srv",
		New: func() kengine.Module { return new(SRVUpstreams) },
	}
}

func (su *SRVUpstreams) Provision(ctx kengine.Context) error {
	su.logger = ctx.Logger()
	if su.Refresh == 0 {
		su.Refresh = kengine.Duration(time.Minute)
	}
...
// KengineModule returns the Kengine module information.
func (AUpstreams) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		ID:  "http.reverse_proxy.upstreams.a",
		New: func() kengine.Module { return new(AUpstreams) },
	}
}

func (au *AUpstreams) Provision(ctx kengine.Context) error {
	au.logger = ctx.Logger()
	if au.Refresh == 0 {
		au.Refresh = kengine.Duration(time.Minute)
	}
	if au.Port == "" {
		au.Port = "80"
	}
...
type MultiUpstreams struct {
	...
	SourcesRaw []json.RawMessage `json:"sources,omitempty" kengine:"namespace=http.reverse_proxy.upstreams inline_key=source"`
...
// KengineModule returns the Kengine module information.
func (MultiUpstreams) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		ID:  "http.reverse_proxy.upstreams.multi",
		New: func() kengine.Module { return new(MultiUpstreams) },
	}
}

func (mu *MultiUpstreams) Provision(ctx kengine.Context) error {
...
type UpstreamResolver struct {
	...
	netAddrs  []kengine.NetworkAddress
}
...
func (u *UpstreamResolver) ParseAddresses() error {
	for _, v := range u.Addresses {
		addr, err := kengine.ParseNetworkAddressWithDefaults(v, "udp", 53)
		if err != nil {
			return err
		}

...
var (
	...
	// typeMap maps kengine load balance configuration to the internal representation of the loadbalance algorithm type.
...
func NewLoadBalancedReverseProxy(lb *LoadBalanced, ctx kengine.Context) error {
	// set defaults
	...
	for _, uc := range lb.Upstreams {
		...
		if uc.CircuitBreaker != nil {
			if _, err := kengine.GetModule(cbModule); err == nil {
...
type LoadBalanced struct {
	...
	// The following struct fields are set by kengine configuration.
...
func (lb *LoadBalanced) Provision(ctx kengine.Context) error {

...
func init() {
	kengine.RegisterModule(HTTPTransport{})
}
...
func (HTTPTransport) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		ID:  "http.reverse_proxy.transport.http",
		New: func() kengine.Module { return new(HTTPTransport) },
	}
}

// Provision sets up h.Transport with a *http.Transport
// that is ready to use.
func (h *HTTPTransport) Provision(ctx kengine.Context) error {
	if len(h.Versions) == 0 {
...
func (h *HTTPTransport) NewTransport(kengineCtx kengine.Context) (*http.Transport, error) {
	// Set keep-alive defaults if it wasn't otherwise configured
	if h.KeepAlive == nil {
		h.KeepAlive = &KeepAlive{
			ProbeInterval:       kengine.Duration(30 * time.Second),
			IdleConnTimeout:     kengine.Duration(2 * time.Minute),
	...
	if h.DialTimeout == 0 {
		h.DialTimeout = kengine.Duration(3 * time.Second)
	}
	...
	if h.LocalAddress != "" {
		netaddr, err := kengine.ParseNetworkAddressWithDefaults(h.LocalAddress, "tcp", 0)
		if err != nil {
			return nil, err
		}
	...
	dialContext := func(ctx context.Context, network, address string) (net.Conn, error) {
		...
		if h.ProxyProtocol != "" {
			proxyProtocolInfo, ok := kenginehttp.GetVar(ctx, proxyProtocolInfoVarKey).(ProxyProtocolInfo)
			if !ok {
			...
			switch h.ProxyProtocol {
			case "v1":
				kengineCtx.Logger().Debug("sending proxy protocol header v1", zap.Any("header", header))
			case "v2":
				kengineCtx.Logger().Debug("sending proxy protocol header v2", zap.Any("header", header))
			}
	...
	if h.ForwardProxyURL != "" {
		...
		kengineCtx.Logger().Info("setting transport proxy url", zap.String("url", h.ForwardProxyURL))
	...
	if h.TLS != nil {
		...
		var err error
		rt.TLSClientConfig, err = h.TLS.MakeTLSClientConfig(kengineCtx)
		if err != nil {
	...
	if !rt.DisableKeepAlives && h.ProxyProtocol != "" {
		kengineCtx.Logger().Warn("disabling keepalives, they are incompatible with using PROXY protocol")
	...
	if len(h.Versions) == 1 && h.Versions[0] == "3" {
		...
		if h.TLS != nil {
			var err error
			h.h3Transport.TLSClientConfig, err = h.TLS.MakeTLSClientConfig(kengineCtx)
			if err != nil {
...
func (h *HTTPTransport) replaceTLSServername(repl *kengine.Replacer) *HTTPTransport {
...
func (t *TLSConfig) MakeTLSClientConfig(ctx kengine.Context) (*tls.Config, error) {
	...
	if t.ClientCertificateAutomate != "" {
		...
		cfg.GetClientCertificate = func(cri *tls.CertificateRequestInfo) (*tls.Certificate, error) {
			certs := kenginetls.AllMatchingCertificates(t.ClientCertificateAutomate)
			var err error
	...
	for _, curveName := range t.Curves {
		curveID := kenginetls.SupportedCurves[curveName]

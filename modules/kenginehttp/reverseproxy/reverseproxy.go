...
func init() {
	kengine.RegisterModule(Handler{})
}
...
type Handler struct {
	...
	TransportRaw json.RawMessage `json:"transport,omitempty" kengine:"namespace=http.reverse_proxy.transport inline_key=protocol"`
	...
	CBRaw json.RawMessage `json:"circuit_breaker,omitempty" kengine:"namespace=http.reverse_proxy.circuit_breakers inline_key=type"`
	...
	DynamicUpstreamsRaw json.RawMessage `json:"dynamic_upstreams,omitempty" kengine:"namespace=http.reverse_proxy.upstreams inline_key=source"`
...
func (Handler) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		...
		New: func() kengine.Module { return new(Handler) },
	}
}

// Provision ensures that h is set up properly before use.
func (h *Handler) Provision(ctx kengine.Context) error {
	eventAppIface, err := ctx.App("events")
	if err != nil {
		return fmt.Errorf("getting events app: %v", err)
	}
	h.events = eventAppIface.(*kengineevents.App)
	h.ctx = ctx
	h.logger = ctx.Logger()
	...
	// (see https://kengine.community/t/set-cookie-manipulation-in-reverse-proxy/7666?u=matt
	...
	if h.LoadBalancing.TryDuration > 0 && h.LoadBalancing.TryInterval == 0 {
		...
		h.LoadBalancing.TryInterval = kengine.Duration(250 * time.Millisecond)
	}
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request, next kenginehttp.Handler) error {
...
func (h *Handler) proxyLoopIteration(r *http.Request, origReq *http.Request, w http.ResponseWriter, proxyErr error, start time.Time, retries int,
	repl *kengine.Replacer, reqHeader http.Header, reqHost string, next kenginehttp.Handler,
) (bool, error) {
	...
	if upstream == nil {
		if proxyErr == nil {
			proxyErr = kenginehttp.Error(http.StatusServiceUnavailable, errNoUpstream)
		}
	...
	kenginehttp.SetVar(r.Context(), dialInfoVarKey, dialInfo)
...
func (h Handler) prepareRequest(req *http.Request, repl *kengine.Replacer) (*http.Request, error) {
	...
	kenginehttp.SetVar(req.Context(), proxyProtocolInfoVarKey, proxyProtocolInfo)
...
func (h Handler) addForwardedHeaders(req *http.Request) error {
	...
	trusted := kenginehttp.GetVar(req.Context(), kenginehttp.TrustedProxyVarKey).(bool)
...
func (h *Handler) reverseProxy(rw http.ResponseWriter, req *http.Request, origReq *http.Request, repl *kengine.Replacer, di DialInfo, next kenginehttp.Handler) error {
	...
	server := req.Context().Value(kenginehttp.ServerCtxKey).(*kenginehttp.Server)
	...
	logger := h.logger.With(
		...
		zap.Object("request", kenginehttp.LoggableHTTPRequest{
			Request:              req,
	...
	if c := logger.Check(zapcore.DebugLevel, "upstream roundtrip"); c != nil {
		c.Write(
			zap.Object("headers", kenginehttp.LoggableHTTPHeader{
				Header:               res.Header,
				ShouldLogCredentials: shouldLogCredentials,
			}),
	...
	if h.HealthChecks != nil && h.HealthChecks.Passive != nil {
		...
		for _, badStatus := range h.HealthChecks.Passive.UnhealthyStatus {
			if kenginehttp.StatusCodeMatches(res.StatusCode, badStatus) {
...
func (h *Handler) finalizeResponse(
	...
	if err != nil {
		...
		// see issue https://github.com/khulnasoft/kengine/issues/5951
...
func (lb LoadBalancing) tryAgain(ctx kengine.Context, start time.Time, retries int, proxyErr error, req *http.Request) bool {
	...
	if proxyErr != nil {
		...
		herr, isHandlerError := proxyErr.(kenginehttp.HandlerError)
...
func statusError(err error) error {
	...
	return kenginehttp.Error(statusCode, err)
}
...
type LoadBalancing struct {
	...
	SelectionPolicyRaw json.RawMessage `json:"selection_policy,omitempty" kengine:"namespace=http.reverse_proxy.selection_policies inline_key=policy"`
...
const proxyHandleResponseContextCtxKey kengine.CtxKey = "reverse_proxy_handle_response_context"
...
var (
	...
	_ kengine.CleanerUpper          = (*Handler)(nil)

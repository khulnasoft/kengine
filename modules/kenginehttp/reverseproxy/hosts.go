...
func (u *Upstream) fillDialInfo(r *http.Request) (DialInfo, error) {
	...
	var addr kengine.NetworkAddress

	// use provided dial address
	var err error
	dial := repl.ReplaceAll(u.Dial, "")
	addr, err = kengine.ParseNetworkAddress(dial)
	if err != nil {
...
// String returns the Kengine network address form
// by joining the network and address with a
// forward slash.
func (di DialInfo) String() string {
	return kengine.JoinNetworkAddress(di.Network, di.Host, di.Port)
}
...
func GetDialInfo(ctx context.Context) (DialInfo, bool) {
	dialInfo, ok := kenginehttp.GetVar(ctx, dialInfoVarKey).(DialInfo)
	return dialInfo, ok
}
...
var hosts = kengine.NewUsagePool()

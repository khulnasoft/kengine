...
func init() {
	httpkenginefile.RegisterHandlerDirective("reverse_proxy", parseKenginefile)
	httpkenginefile.RegisterHandlerDirective("copy_response", parseCopyResponseKenginefile)
	httpkenginefile.RegisterHandlerDirective("copy_response_headers", parseCopyResponseHeadersKenginefile)
}

func parseKenginefile(h httpkenginefile.Helper) (kenginehttp.MiddlewareHandler, error) {
	rp := new(Handler)
	err := rp.UnmarshalKenginefile(h.Dispenser)
	if err != nil {
		return nil, err
	}
	err = rp.FinalizeUnmarshalKenginefile(h)
	if err != nil {
		return nil, err
	}
	return rp, nil
...
func (h *Handler) UnmarshalKenginefile(d *kenginefile.Dispenser) error {
	...
	h.responseMatchers = make(map[string]kenginehttp.ResponseMatcher)
	...
	appendUpstream := func(address string) error {
		...
		parsedAddr, err := kengine.ParseNetworkAddress(pa.dialAddr())
		if err != nil {
			return d.WrapErr(err)
		}
	...
	for d.NextBlock(0) {
		...
		if strings.HasPrefix(d.Val(), matcherPrefix) {
			err := kenginehttp.ParseNamedResponseMatcher(d.NewFromNextSegment(), h.responseMatchers)
			if err != nil {
				return err
			}
			continue
		...
		case "dynamic":
			...
			h.DynamicUpstreamsRaw = kengineconfig.JSONModuleObject(source, "source", dynModule, nil)
		...
		case "lb_policy":
			...
			h.LoadBalancing.SelectionPolicyRaw = kengineconfig.JSONModuleObject(sel, "policy", name, nil)
		...
		case "lb_try_duration":
			...
			h.LoadBalancing.TryDuration = kengine.Duration(dur)
		...
		case "lb_try_interval":
			...
			h.LoadBalancing.TryInterval = kengine.Duration(dur)

		case "lb_retry_match":
			matcherSet, err := kenginehttp.ParseKenginefileNestedMatcherSet(d)
			if err != nil {
		...
		case "health_path":
			...
			kengine.Log().Named("config.adapter.kenginefile").Warn("the 'health_path' subdirective is deprecated, please use 'health_uri' instead!")
		...
		case "health_interval":
			...
			h.HealthChecks.Active.Interval = kengine.Duration(dur)
		...
		case "health_timeout":
			...
			h.HealthChecks.Active.Timeout = kengine.Duration(dur)
		...
		case "fail_duration":
			...
			h.HealthChecks.Passive.FailDuration = kengine.Duration(dur)
		...
		case "unhealthy_latency":
			...
			h.HealthChecks.Passive.UnhealthyLatency = kengine.Duration(dur)
		...
		case "flush_interval":
			...
			if fi, err := strconv.Atoi(d.Val()); err == nil {
				h.FlushInterval = kengine.Duration(fi)
			} else {
				dur, err := kengine.ParseDuration(d.Val())
				if err != nil {
					return d.Errf("bad duration value '%s': %v", d.Val(), err)
				}
				h.FlushInterval = kengine.Duration(dur)
			}
		...
		case "stream_timeout":
			...
			if fi, err := strconv.Atoi(d.Val()); err == nil {
				h.StreamTimeout = kengine.Duration(fi)
			} else {
				dur, err := kengine.ParseDuration(d.Val())
				if err != nil {
					return d.Errf("bad duration value '%s': %v", d.Val(), err)
				}
				h.StreamTimeout = kengine.Duration(dur)
			}
		...
		case "stream_close_delay":
			...
			if fi, err := strconv.Atoi(d.Val()); err == nil {
				h.StreamCloseDelay = kengine.Duration(fi)
			} else {
				dur, err := kengine.ParseDuration(d.Val())
				if err != nil {
					return d.Errf("bad duration value '%s': %v", d.Val(), err)
				}
				h.StreamCloseDelay = kengine.Duration(dur)
			}
		...
		case "header_up":
			...
			switch len(args) {
			case 1:
				err = headers.KenginefileHeaderOp(h.Headers.Request, args[0], "", nil)
			case 2:
				// some lint checks, I guess
				if strings.EqualFold(args[0], "host") && (args[1] == "{hostport}" || args[1] == "{http.request.hostport}") {
					kengine.Log().Named("kenginefile").Warn("Unnecessary header_up Host: the reverse proxy's default behavior is to pass headers to the upstream")
				}
				if strings.EqualFold(args[0], "x-forwarded-for") && (args[1] == "{remote}" || args[1] == "{http.request.remote}" || args[1] == "{remote_host}" || args[1] == "{http.request.remote.host}") {
					kengine.Log().Named("kenginefile").Warn("Unnecessary header_up X-Forwarded-For: the reverse proxy's default behavior is to pass headers to the upstream")
				}
				if strings.EqualFold(args[0], "x-forwarded-proto") && (args[1] == "{scheme}" || args[1] == "{http.request.scheme}") {
					kengine.Log().Named("kenginefile").Warn("Unnecessary header_up X-Forwarded-Proto: the reverse proxy's default behavior is to pass headers to the upstream")
				}
				if strings.EqualFold(args[0], "x-forwarded-host") && (args[1] == "{host}" || args[1] == "{http.request.host}" || args[1] == "{hostport}" || args[1] == "{http.request.hostport}") {
					kengine.Log().Named("kenginefile").Warn("Unnecessary header_up X-Forwarded-Host: the reverse proxy's default behavior is to pass headers to the upstream")
				}
				err = headers.KenginefileHeaderOp(h.Headers.Request, args[0], args[1], nil)
			case 3:
				err = headers.KenginefileHeaderOp(h.Headers.Request, args[0], args[1], &args[2])
			default:
				return d.ArgErr()
			}
		...
		case "header_down":
			...
			switch len(args) {
			case 1:
				err = headers.KenginefileHeaderOp(h.Headers.Response.HeaderOps, args[0], "", nil)
			case 2:
				err = headers.KenginefileHeaderOp(h.Headers.Response.HeaderOps, args[0], args[1], nil)
			case 3:
				err = headers.KenginefileHeaderOp(h.Headers.Response.HeaderOps, args[0], args[1], &args[2])
			default:
				return d.ArgErr()
			}
	...
	if transport != nil {
		...
		if !reflect.DeepEqual(transport, new(HTTPTransport)) {
			h.TransportRaw = kengineconfig.JSONModuleObject(transport, "protocol", transportModuleName, nil)
		}
	}
...
func (h *Handler) FinalizeUnmarshalKenginefile(helper httpkenginefile.Helper) error {
...
// UnmarshalKenginefile deserializes Kenginefile tokens into h.
//
//	transport http {
//	    read_buffer             <size>
//	    write_buffer            <size>
...
//	}
func (h *HTTPTransport) UnmarshalKenginefile(d *kenginefile.Dispenser) error {
	...
	for d.NextBlock(0) {
		...
		case "read_timeout":
			...
			h.ReadTimeout = kengine.Duration(timeout)
		...
		case "write_timeout":
			...
			h.WriteTimeout = kengine.Duration(timeout)
		...
		case "dial_timeout":
			...
			h.DialTimeout = kengine.Duration(dur)
		...
		case "dial_fallback_delay":
			...
			h.FallbackDelay = kengine.Duration(dur)
		...
		case "response_header_timeout":
			...
			h.ResponseHeaderTimeout = kengine.Duration(dur)
		...
		case "expect_continue_timeout":
			...
			h.ExpectContinueTimeout = kengine.Duration(dur)
		...
		case "tls_timeout":
			...
			h.TLS.HandshakeTimeout = kengine.Duration(dur)

		case "tls_trusted_ca_certs":
			kengine.Log().Warn("The 'tls_trusted_ca_certs' field is deprecated. Use the 'tls_trust_pool' field instead.")
			args := d.RemainingArgs()
			if len(args) == 0 {
				return d.ArgErr()
			}
		...
		case "keepalive":
			...
			h.KeepAlive.IdleConnTimeout = kengine.Duration(dur)
		...
		case "keepalive_interval":
			...
			h.KeepAlive.ProbeInterval = kengine.Duration(dur)
		...
		case "tls_trust_pool":
			...
			if h.TLS.CARaw != nil {
				return d.Err("cannot specify \"tls_trust_pool\" twice in kenginefile")
			}
			h.TLS.CARaw = kengineconfig.JSONModuleObject(ca, "provider", modStem, nil)
		case "local_address":
			if !d.NextArg() {
				return d.ArgErr()
			}
...
func parseCopyResponseKenginefile(h httpkenginefile.Helper) (kenginehttp.MiddlewareHandler, error) {
	crh := new(CopyResponseHandler)
	err := crh.UnmarshalKenginefile(h.Dispenser)
	if err != nil {
		return nil, err
	}
	return crh, nil
...
//	    status <status>
//	}
func (h *CopyResponseHandler) UnmarshalKenginefile(d *kenginefile.Dispenser) error {
	d.Next() // consume directive name

	args := d.RemainingArgs()
	if len(args) == 1 {
		if num, err := strconv.Atoi(args[0]); err == nil && num > 0 {
			h.StatusCode = kenginehttp.WeakString(args[0])
			return nil
		}
	}
	...
	for d.NextBlock(0) {
		...
		case "status":
			...
			h.StatusCode = kenginehttp.WeakString(d.Val())
		default:
			return d.Errf("unrecognized subdirective '%s'", d.Val())
		}
	}
...
func parseCopyResponseHeadersKenginefile(h httpkenginefile.Helper) (kenginehttp.MiddlewareHandler, error) {
...
//	    exclude <fields...>
//	}
func (h *CopyResponseHeadersHandler) UnmarshalKenginefile(d *kenginefile.Dispenser) error {
	d.Next() // consume directive name
...
//	}
func (u *SRVUpstreams) UnmarshalKenginefile(d *kenginefile.Dispenser) error {
	...
	for d.NextBlock(0) {
		...
		case "refresh":
			...
			u.Refresh = kengine.Duration(dur)
		...
		case "dial_timeout":
			...
			u.DialTimeout = kengine.Duration(dur)
		...
		case "dial_fallback_delay":
			...
			u.FallbackDelay = kengine.Duration(dur)
		case "grace_period":
			if !d.NextArg() {
				return d.ArgErr()
			}
			...
			u.GracePeriod = kengine.Duration(dur)
		default:
...
//	    versions            ipv4|ipv6
//	}
func (u *AUpstreams) UnmarshalKenginefile(d *kenginefile.Dispenser) error {
	d.Next() // consume upstream source name
...
//	    <source> [...]
//	}
func (u *MultiUpstreams) UnmarshalKenginefile(d *kenginefile.Dispenser) error {
	d.Next() // consume upstream source name
	...
	for d.NextBlock(0) {
		...
		u.SourcesRaw = append(u.SourcesRaw, kengineconfig.JSONModuleObject(source, "source", dynModule, nil))
	}
	return nil
}
...
// Interface guards
var (
	_ kenginefile.Unmarshaler = (*Handler)(nil)
	_ kenginefile.Unmarshaler = (*HTTPTransport)(nil)
	_ kenginefile.Unmarshaler = (*SRVUpstreams)(nil)
	_ kenginefile.Unmarshaler = (*AUpstreams)(nil)
	_ kenginefile.Unmarshaler = (*MultiUpstreams)(nil)
)

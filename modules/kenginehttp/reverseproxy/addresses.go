...
func (p parsedAddr) dialAddr() string {
	...
	if p.network != "" {
		return kengine.JoinNetworkAddress(p.network, p.host, p.port)
	}
...
func (p parsedAddr) isUnix() bool {
	return kengine.IsUnixNetwork(p.network)
}
...
func parseUpstreamDialAddress(upstreamAddr string) (parsedAddr, error) {
	...
	if strings.Contains(upstreamAddr, "://") {
		...
		var err error
		network, host, port, err = kengine.SplitNetworkAddress(upstreamAddr)
		if err != nil {
			host = upstreamAddr
		}
		...
		if port == "" && !strings.Contains(host, "{") && !kengine.IsUnixNetwork(network) {
			port = "80"
		}
	}

...
type Server struct {
	...
	listenerWrappers    []kengine.ListenerWrapper
	...
	addresses   []kengine.NetworkAddress
...
func (s *Server) hasListenerAddress(fullAddr string) bool {
	laddrs, err := kengine.ParseNetworkAddress(fullAddr)
	if err != nil {
		return false
	}
	...
	for _, lnAddr := range s.Listen {
		thisAddrs, err := kengine.ParseNetworkAddress(lnAddr)
		if err != nil {
			continue
		}
...
func (s *Server) hasTLSClientAuth() bool {
	return slices.ContainsFunc(s.TLSConnPolicies, func(cp *kenginetls.ConnectionPolicy) bool {
...
func (s *Server) findLastRouteWithHostMatcher() int {
	...
	// See https://kengine.community/t/how-to-set-priority-in-kenginefile/13002/8
	if !foundHostMatcher {
		return 0
	}
...
func (s *Server) serveHTTP3(addr kengine.NetworkAddress, tlsCfg *tls.Config) error {
	...
	if s.h3server == nil {
		s.h3server = &http3.Server{
			// Currently when closing a http3.Server, only listeners are closed. But kengine reuses these listeners
			// if possible, requests are still read and handled by the old handler. Close these connections manually.
			// see issue: https://github.com/khulnasoft/kengine/issues/6195
...
func (s *Server) logTrace(mh MiddlewareHandler) {
	...
	if c := s.traceLogger.Check(zapcore.DebugLevel, kengine.GetModuleName(mh)); c != nil {
...
func (s *Server) logRequest(
	...
	repl *kengine.Replacer, bodyReader *lengthReader, shouldLogCredentials bool,
) {
...
// PrepareRequest fills the request r for use in a Kengine HTTP handler chain. w and s can
...
const (
	...
	quicConnCtxKey kengine.CtxKey = "quic_conn"

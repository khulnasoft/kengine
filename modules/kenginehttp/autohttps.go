...
func (app *App) automaticHTTPSPhase1(ctx kengine.Context, repl *kengine.Replacer) error {
	...
	// https://github.com/khulnasoft/kengine/issues/3443)
	redirDomains := make(map[string][]kengine.NetworkAddress)
	...
	for srvName, srv := range app.Servers {
		...
		if srv.TLSConnPolicies == nil &&
			...
			srv.TLSConnPolicies = kenginetls.ConnectionPolicies{new(kenginetls.ConnectionPolicy)}
		}
		...
		for _, listenAddr := range srv.Listen {
			...
			addr, err := kengine.ParseNetworkAddress(listenAddr)
			if err != nil {
...
uniqueDomainsLoop:
				...
				for _, apHost := range ap.Subjects() {
					if apHost == d {
						...
						if len(ap.Issuers) == 0 && ap.AllInternalSubjects() {
							iss := new(kenginetls.InternalIssuer)
	...
	for addrStr, domains := range domainsByAddr {
		...
		addr, err := kengine.ParseNetworkAddress(addrStr)
		if err != nil {
			return err
		}
...
redirServersLoop:
	for redirServerAddr, routes := range redirServers {
		...
		for _, srv := range app.Servers {
			...
			// see https://github.com/khulnasoft/kengine/issues/3212
...
func (app *App) makeRedirRoute(redirToPort uint, matcherSet MatcherSet) Route {
	...
	// notably, also not the port that Kengine thinks is the HTTPS port (the
...
func (app *App) createAutomationPolicies(ctx kengine.Context, internalNames, tailscaleNames []string) error {
	...
	// those defaults, too
	var basePolicy *kenginetls.AutomationPolicy
	var foundBasePolicy bool
	if app.tlsApp.Automation == nil {
		// we will expect this to not be nil from now on
		app.tlsApp.Automation = new(kenginetls.AutomationConfig)
	}
	for _, ap := range app.tlsApp.Automation.Policies {
		...
		if ap.Issuers == nil {
			var err error
			ap.Issuers, err = kenginetls.DefaultIssuersProvisioned(ctx)
			if err != nil {
				return err
			}
		}
	...
	if basePolicy == nil {
		...
		basePolicy = new(kenginetls.AutomationPolicy)
	}
	...
	var baseACMEIssuer *kenginetls.ACMEIssuer
	...
	if baseACMEIssuer == nil {
		...
		baseACMEIssuer = new(kenginetls.ACMEIssuer)
	}
	...
	if basePolicy.Issuers == nil {
		var err error
		basePolicy.Issuers, err = kenginetls.DefaultIssuersProvisioned(ctx)
		if err != nil {
			return err
		}
		for _, iss := range basePolicy.Issuers {
	...
	if len(internalNames) > 0 {
		...
		// is easy to do; consider the case of a Kenginefile
...
func (app *App) fillInACMEIssuer(acmeIssuer *kenginetls.ACMEIssuer) error {

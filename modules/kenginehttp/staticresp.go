...
func init() {
	kengine.RegisterModule(StaticResponse{})
...
`,
		CobraFunc: func(cmd *cobra.Command) {
			...
			cmd.RunE = kenginecmd.WrapCommandFuncForCobra(cmdRespond)
		},
	})
}
...
func (StaticResponse) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		...
		New: func() kengine.Module { return new(StaticResponse) },
	}
}
...
func buildHTTPServer(i int, port uint, addr string, statusCode int, hdr http.Header, body string, accessLog bool) (*Server, error) {
	...
	handlers = append(handlers, kengineconfig.JSONModuleObject(handler, "handler", "static_response", nil))
...
func cmdRespond(fl kenginecmd.Flags) (int, error) {
	kengine.TrapSignals()
	...
	if fl.NArg() > 1 {
		return kengine.ExitCodeFailedStartup, fmt.Errorf("too many unflagged arguments")
	}
	...
	if arg != "" {
		...
		if bodyFl != "" && statusCodeFlagSpecified {
			return kengine.ExitCodeFailedStartup, fmt.Errorf("unflagged argument \"%s\" is overridden by flags", arg)
		}
	...
	for i, h := range headers {
		...
		if !found || key == "" || val == "" {
			return kengine.ExitCodeFailedStartup, fmt.Errorf("header %d: invalid format \"%s\" (expecting \"Field: value\")", i, h)
		}
		hdr.Set(key, val)
	}
	...
	listenAddr, err := kengine.ParseNetworkAddress(listen)
	if err != nil {
		return kengine.ExitCodeFailedStartup, err
	}
	...
	loadedHTTPApp, err := kengine.ActiveContext().App("http")
	if err != nil {
		return kengine.ExitCodeFailedStartup, err
	}

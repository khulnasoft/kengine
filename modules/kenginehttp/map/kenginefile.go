...
func init() {
	httpkenginefile.RegisterHandlerDirective("map", parseKenginefile)
}

// parseKenginefile sets up the map handler from Kenginefile tokens. Syntax:
//
...
// regular expression.
//
// The Kenginefile adapter treats outputs that are a literal hyphen (-) as a null/nil
...
func parseKenginefile(h httpkenginefile.Helper) (kenginehttp.MiddlewareHandler, error) {
	...
	for _, dest := range handler.Destinations {
		if shorthand := httpkenginefile.WasReplacedPlaceholderShorthand(dest); shorthand != "" {
			return nil, h.Errf("destination %s conflicts with a Kenginefile placeholder shorthand", shorthand)
		}
	}

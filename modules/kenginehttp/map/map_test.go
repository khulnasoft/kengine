...
func TestHandler(t *testing.T) {
	for i, tc := range []struct {
		...
		{
			// Test case from https://kengine.community/t/map-directive-and-regular-expressions/13866/14?u=matt
		...
		if err := tc.handler.Provision(kengine.Context{}); err != nil {
		...
		repl := kenginehttp.NewTestReplacer(req)
		repl.Set("testvar", "testing")
		ctx := context.WithValue(req.Context(), kengine.ReplacerCtxKey, repl)
		req = req.WithContext(ctx)

		rr := httptest.NewRecorder()
		noop := kenginehttp.HandlerFunc(func(_ http.ResponseWriter, _ *http.Request) error { return nil })

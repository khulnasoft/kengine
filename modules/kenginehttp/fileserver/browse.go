...
// if you are running Kengine via config, you can specify a
...
func (fsrv *FileServer) serveBrowse(fileSystem fs.FS, root, dirPath string, w http.ResponseWriter, r *http.Request, next kenginehttp.Handler) error {
	...
	// See https://github.com/khulnasoft/kengine/issues/4466.
...
func (fsrv *FileServer) loadDirectoryContents(ctx context.Context, fileSystem fs.FS, dir fs.ReadDirFile, root, urlPath string, repl *kengine.Replacer) (*browseTemplateContext, error) {
...
func (fsrv *FileServer) browseApplyQueryParams(w http.ResponseWriter, r *http.Request, listing *browseTemplateContext) {
	...
	// The configs in Kenginefile have lower priority than Query params,
	// so put it at first.
...
func (fsrv *FileServer) isSymlinkTargetDir(fileSystem fs.FS, f fs.FileInfo, root, urlPath string) bool {
	...
	target := kenginehttp.SanitizedPathJoin(root, path.Join(urlPath, f.Name()))

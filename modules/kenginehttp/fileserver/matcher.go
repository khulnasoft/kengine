...
func init() {
	kengine.RegisterModule(MatchFile{})
}
...
func (MatchFile) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		ID:  "http.matchers.file",
		New: func() kengine.Module { return new(MatchFile) },
	}
}

// UnmarshalKenginefile sets up the matcher from Kenginefile tokens. Syntax:
//
//	file <files...> {
//	    root      <path>
//	    try_files <files...>
...
//	}
func (m *MatchFile) UnmarshalKenginefile(d *kenginefile.Dispenser) error {
...
func (MatchFile) CELLibrary(ctx kengine.Context) (cel.Library, error) {
	requestType := cel.ObjectType("http.Request")

	matcherFactory := func(data ref.Val) (kenginehttp.RequestMatcher, error) {
		values, err := kenginehttp.CELValueToMapStrList(data)
		if err != nil {
			return nil, err
		}
	...
	envOptions := []cel.EnvOption{
		...
		cel.Function("file", cel.Overload("file_request_map", []*cel.Type{requestType, kenginehttp.CELTypeJSON}, cel.BoolType)),
		cel.Function("file_request_map",
			cel.Overload("file_request_map", []*cel.Type{requestType, kenginehttp.CELTypeJSON}, cel.BoolType),
			cel.SingletonBinaryBinding(kenginehttp.CELMatcherRuntimeFunction("file_request_map", matcherFactory))),
	}

	programOptions := []cel.ProgramOption{
		cel.CustomDecorator(kenginehttp.CELMatcherDecorator("file_request_map", matcherFactory)),
	}

	return kenginehttp.NewMatcherCELLibrary(envOptions, programOptions), nil
}
...
func celFileMatcherMacroExpander() parser.MacroExpander {
	return func(eh parser.ExprHelper, target ast.Expr, args []ast.Expr) (ast.Expr, *common.Error) {
		...
		if len(args) == 1 {
			arg := args[0]
			if isCELStringLiteral(arg) || isCELKenginePlaceholderCall(arg) {
				return eh.NewCall("file",
					eh.NewIdent("request"),
...
func (m *MatchFile) Provision(ctx kengine.Context) error {
	m.logger = ctx.Logger()
...
func (m MatchFile) selectFile(r *http.Request) (matched bool) {
	...
	makeCandidates := func(file string) []matchCandidate {
		...
		fullPattern := kenginehttp.SanitizedPathJoin(root, beforeSplit)
	...
	case "", tryPolicyFirstExist:
		for _, pattern := range m.TryFiles {
			if err := parseErrorCode(pattern); err != nil {
				kenginehttp.SetVar(r.Context(), kenginehttp.MatcherErrorVarKey, err)
				return
			}
...
func parseErrorCode(input string) error {
	if len(input) > 1 && input[0] == '=' {
		...
		return kenginehttp.Error(code, fmt.Errorf("%s", input[1:]))
	}
	return nil
}
...
// Interface guards
var (
	_ kengine.Validator              = (*MatchFile)(nil)
	_ kenginehttp.RequestMatcher     = (*MatchFile)(nil)
	_ kenginehttp.CELLibraryProducer = (*MatchFile)(nil)
)

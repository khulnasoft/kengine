...
func init() {
	httpkenginefile.RegisterHandlerDirective("file_server", parseKenginefile)
	httpkenginefile.RegisterDirective("try_files", parseTryFiles)
}

// parseKenginefile parses the file_server directive.
// See UnmarshalKenginefile for the syntax.
func parseKenginefile(h httpkenginefile.Helper) (kenginehttp.MiddlewareHandler, error) {
	fsrv := new(FileServer)
	err := fsrv.UnmarshalKenginefile(h.Dispenser)
	if err != nil {
		return fsrv, err
	}
	err = fsrv.FinalizeUnmarshalKenginefile(h)
	if err != nil {
		return nil, err
	}
	return fsrv, err
...
// UnmarshalKenginefile parses the file_server directive. It enables
...
// to finalize setup of hidden Kenginefiles.
func (fsrv *FileServer) UnmarshalKenginefile(d *kenginefile.Dispenser) error {
	...
	for d.NextBlock(0) {
		...
		case "precompressed":
			...
			for d.NextArg() {
				...
				if fsrv.PrecompressedRaw == nil {
					fsrv.PrecompressedRaw = make(kengine.ModuleMap)
				}
				fsrv.PrecompressedRaw[d.Val()] = kengineconfig.JSON(precompress, nil)
		...
		case "status":
			...
			fsrv.StatusCode = kenginehttp.WeakString(d.Val())
...
// requires having an httpkenginefile.Helper to function, to setup hidden Kenginefiles.
func (fsrv *FileServer) FinalizeUnmarshalKenginefile(h httpkenginefile.Helper) error {
	// Hide the Kenginefile (and any imported Kenginefiles).
	// This needs to be done in here instead of UnmarshalKenginefile
	// because UnmarshalKenginefile only has access to the dispenser
	// and not the helper, and only the helper has access to the
	// Kenginefiles function.
	if configFiles := h.Kenginefiles(); len(configFiles) > 0 {
...
func parseTryFiles(h httpkenginefile.Helper) ([]httpkenginefile.ConfigValue, error) {
	if !h.Next() {
		return nil, h.ArgErr()
	}
...
var _ kenginefile.Unmarshaler = (*FileServer)(nil)

...
// FileServer implements a static file server responder for Kengine.
type FileServer struct {
	...
	Fallback        kenginehttp.RouteList `json:"fallback,omitempty"`
...
func (fsrv *FileServer) ServeHTTP(w http.ResponseWriter, r *http.Request) error {
	...
	if err != nil {
		...
		if os.IsNotExist(err) {
			return kenginehttp.Error(http.StatusNotFound, err)
...
// is returned.
func (fsrv *FileServer) selectFile(r *http.Request, repl kengine.Replacer, filesToHide []string) string {
...
// Interface guard
var _ kenginehttp.Handler = (*FileServer)(nil)

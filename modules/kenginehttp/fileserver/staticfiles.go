...
func init() {
	kengine.RegisterModule(FileServer{})
}
...
// Kengine will attempt to serve an index file if present. For example,
// a request to `/dir/` will attempt to serve `/dir/index.html` if
// it exists. The index file names to try are configurable. If a
// requested directory does not have an index file, Kengine writes a
...
// Kengine may serve a JSON array of the directory listing when the `Accept`
...
type FileServer struct {
	// The file system implementation to use. By default, Kengine uses the local
	// disk file system.
	//
	...
	PrecompressedRaw kengine.ModuleMap `json:"precompressed,omitempty" kengine:"namespace=http.precompressed"`
	...
	fsmap kengine.FileSystems

	logger *zap.Logger
}

// KengineModule returns the Kengine module information.
func (FileServer) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		ID:  "http.handlers.file_server",
		New: func() kengine.Module { return new(FileServer) },
	}
}

// Provision sets up the static files responder.
func (fsrv *FileServer) Provision(ctx kengine.Context) error {
...
func (fsrv *FileServer) ServeHTTP(w http.ResponseWriter, r *http.Request, next kenginehttp.Handler) error {
	repl := r.Context().Value(kengine.ReplacerCtxKey).(*kengine.Replacer)

	if runtime.GOOS == "windows" {
		// reject paths with Alternate Data Streams (ADS)
		if strings.Contains(r.URL.Path, ":") {
			return kenginehttp.Error(http.StatusBadRequest, fmt.Errorf("illegal ADS path"))
		}
		...
		if len(path.Base(trimmedPath)) <= 12 && strings.Contains(trimmedPath, "~") {
			return kenginehttp.Error(http.StatusBadRequest, fmt.Errorf("illegal short name"))
		}
	...
	if !ok {
		return kenginehttp.Error(http.StatusNotFound, fmt.Errorf("filesystem not found"))
	}

	// remove any trailing `/` as it breaks fs.ValidPath() in the stdlib
	filename := strings.TrimSuffix(kenginehttp.SanitizedPathJoin(root, r.URL.Path), "/")
	...
	if err != nil {
		...
		if errors.Is(err, fs.ErrNotExist) || errors.Is(err, fs.ErrInvalid) {
			...
			return kenginehttp.Error(http.StatusForbidden, err)
		}
	...
	if info.IsDir() && len(fsrv.IndexNames) > 0 {
		for _, indexPage := range fsrv.IndexNames {
			...
			indexPath := kenginehttp.SanitizedPathJoin(filename, indexPage)
	...
	// in HTML (see https://github.com/khulnasoft/kengine/issues/2741)
	if fsrv.CanonicalURIs == nil || *fsrv.CanonicalURIs {
		...
		// See https://github.com/khulnasoft/kengine/issues/4205.
		origReq := r.Context().Value(kenginehttp.OriginalRequestCtxKey).(http.Request)
	...
	for _, ae := range encode.AcceptedEncodings(r, fsrv.PrecompressedOrder) {
		...
		if err != nil {
			...
			if kengineErr, ok := err.(kenginehttp.HandlerError); ok && kengineErr.StatusCode == http.StatusServiceUnavailable {
				return err
			}
			file = nil
			continue
		...
		// https://kengine.community/t/gzipped-sidecar-file-wrong-same-etag/16793
		if etag == "" {
	...
	if file == nil {
		...
		if err != nil {
			if herr, ok := err.(kenginehttp.HandlerError); ok &&
	...
	if r.Method != http.MethodGet && r.Method != http.MethodHead {
		...
		if _, ok := r.Context().Value(kenginehttp.ErrorCtxKey).(error); !ok {
			respHeader.Add("Allow", "GET, HEAD")
			return kenginehttp.Error(http.StatusMethodNotAllowed, nil)
		}
	}
	...
	if reqErr, ok := r.Context().Value(kenginehttp.ErrorCtxKey).(error); ok {
		statusCodeOverride = http.StatusInternalServerError
		if handlerErr, ok := reqErr.(kenginehttp.HandlerError); ok {
...
func (fsrv *FileServer) openFile(fileSystem fs.FS, filename string, w http.ResponseWriter) (fs.File, error) {
	...
	if err != nil {
		...
		if errors.Is(err, fs.ErrNotExist) {
			...
			return nil, kenginehttp.Error(http.StatusNotFound, err)
		} else if errors.Is(err, fs.ErrPermission) {
			if c := fsrv.logger.Check(zapcore.DebugLevel, "permission denied"); c != nil {
				c.Write(zap.String("filename", filename), zap.Error(err))
			}
			return nil, kenginehttp.Error(http.StatusForbidden, err)
		}
		...
		return nil, kenginehttp.Error(http.StatusServiceUnavailable, err)
	}
	return file, nil
}
...
func (fsrv *FileServer) transformHidePaths(repl *kengine.Replacer) []string {
...
func (fsrv *FileServer) notFound(w http.ResponseWriter, r *http.Request, next kenginehttp.Handler) error {
	if fsrv.PassThru {
		return next.ServeHTTP(w, r)
	}
	return kenginehttp.Error(http.StatusNotFound, nil)
}
...
// of Kengine deployments, stores mod times with millisecond precision,

...
func TestPHPFileMatcher(t *testing.T) {
	for i, tc := range []struct {
		...
		{
			// See https://github.com/khulnasoft/kengine/issues/3623
			path:         "/%E2%C3",
			expectedPath: "/%E2%C3",
			expectedType: "file",
			matched:      false,
...
var expressionTests = []struct {
	name              string
	expression        *kenginehttp.MatchExpression
	urlTarget         string
	httpMethod        string
...
func TestMatchExpressionMatch(t *testing.T) {
	for _, tst := range expressionTests {
		...
		t.Run(tc.name, func(t *testing.T) {
			kengineCtx, cancel := kengine.NewContext(kengine.Context{Context: context.Background()})
			defer cancel()
			err := tc.expression.Provision(kengineCtx)
			if err != nil {
				if !tc.wantErr {

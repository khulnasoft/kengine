...
func init() {
	kengine.RegisterModule(Rewrite{})
}
...
func (Rewrite) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		ID:  "http.handlers.rewrite",
		New: func() kengine.Module { return new(Rewrite) },
	}
}

// Provision sets up rewr.
func (rewr *Rewrite) Provision(ctx kengine.Context) error {
...
func (rewr Rewrite) ServeHTTP(w http.ResponseWriter, r *http.Request, next kenginehttp.Handler) error {
	...
	if changed {
		c.Write(
			zap.Object("request", kenginehttp.LoggableHTTPRequest{Request: r}),
...
func (rewr Rewrite) Rewrite(r *http.Request, repl *kengine.Replacer) bool {
	oldMethod := r.Method
	oldURI := r.RequestURI
	...
	if rewr.StripPathPrefix != "" {
		...
		changePath(r, func(escapedPath string) string {
			escapedPath = kenginehttp.CleanPath(escapedPath, mergeSlashes)
...
func buildQueryString(qs string, repl *kengine.Replacer) string {
...
func (rep substrReplacer) do(r *http.Request, repl *kengine.Replacer) {
	if rep.Find == "" {
		return
	}
	...
	changePath(r, func(pathOrRawPath string) string {
		return strings.Replace(kenginehttp.CleanPath(pathOrRawPath, mergeSlashes), find, replace, lim)
	})
...
func (rep regexReplacer) do(r *http.Request, repl *kengine.Replacer) {
...
func (replacement *queryOpsReplacement) Provision(_ kengine.Context) error {
...
func (q *queryOps) do(r *http.Request, repl *kengine.Replacer) {
	query := r.URL.Query()

...
func init() {
	kengine.RegisterModule(Authentication{})
}
...
func (Authentication) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		...
		New: func() kengine.Module { return new(Authentication) },
	}
}
...
func (a Authentication) ServeHTTP(w http.ResponseWriter, r *http.Request, next kenginehttp.Handler) error {
	...
	if !authed {
		return kenginehttp.Error(http.StatusUnauthorized, fmt.Errorf("not authenticated"))
	}
...
// Interface guards
var (
	_ kengine.Provisioner           = (*Authentication)(nil)
	_ kenginehttp.MiddlewareHandler = (*Authentication)(nil)
)

...
import (
	...
	"github.com/khulnasoft/kengine/pkg/kenginescript"
...
func (m *MatchNegate) Provision(ctx kengine.Context) error {
...
func (m MatchStarlarkExpr) Match(r *http.Request) bool {
	...
	env := kenginescript.MatcherEnv(r)
	val, err := starlark.Eval(thread, "", input, env)
	if err != nil {
		// TODO: Can we detect this in Provision or Validate instead?
		log.Printf("kenginescript for matcher is invalid: attempting to evaluate expression `%v` error `%v`", input, err)
		return false
	}
...
func (mre *MatchRegexp) Match(input string, repl kengine.Replacer, scope string) bool {
...
var (
	...
	_ kengine.Provisioner = (*MatchNegate)(nil)

...
func init() {
	kengine.RegisterModule(Templates{})
}
...
//
// In addition to the standard functions and the Sprig library, Kengine adds
...
type Templates struct {
	...
	ExtensionsRaw kengine.ModuleMap `json:"match,omitempty" kengine:"namespace=http.handlers.templates.functions"`
...
func (Templates) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		ID:  "http.handlers.templates",
		New: func() kengine.Module { return new(Templates) },
	}
}

// Provision provisions t.
func (t *Templates) Provision(ctx kengine.Context) error {
	t.logger = ctx.Logger()
...
func (t *Templates) ServeHTTP(w http.ResponseWriter, r *http.Request, next kenginehttp.Handler) error {
	...
	rec := kenginehttp.NewResponseRecorder(w, buf, shouldBuf)
...
func (t *Templates) executeTemplate(rr kenginehttp.ResponseRecorder, r *http.Request) error {
	...
	if err != nil {
		...
		var handlerErr kenginehttp.HandlerError
...
// Interface guards
var (
	_ kengine.Provisioner           = (*Templates)(nil)
	_ kengine.Validator             = (*Templates)(nil)
	_ kenginehttp.MiddlewareHandler = (*Templates)(nil)
)

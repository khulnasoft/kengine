...
func (c TemplateContext) OriginalReq() http.Request {
	or, _ := c.Req.Context().Value(kenginehttp.OriginalRequestCtxKey).(http.Request)
	return or
}
...
func (c TemplateContext) funcHTTPInclude(uri string) (string, error) {
	...
	virtReq.RemoteAddr = "127.0.0.1:10000" // https://github.com/khulnasoft/kengine/issues/5835
	virtReq.Header = c.Req.Header.Clone()
	virtReq.Header.Set("Accept-Encoding", "identity") // https://github.com/khulnasoft/kengine/issues/4352
	...
	server := c.Req.Context().Value(kenginehttp.ServerCtxKey).(http.Handler)
...
func (c TemplateContext) funcPlaceholder(name string) string {
	repl := c.Req.Context().Value(kengine.ReplacerCtxKey).(*kengine.Replacer)
...
func (c TemplateContext) ClientIP() string {
	address := kenginehttp.GetVar(c.Req.Context(), kenginehttp.ClientIPVarKey).(string)
...
func (TemplateContext) funcMarkdown(input any) (string, error) {
	inputStr := kengine.ToString(input)
...
func (TemplateContext) funcSplitFrontMatter(input any) (parsedMarkdownDoc, error) {
	meta, body, err := extractFrontMatter(kengine.ToString(input))
	if err != nil {
...
func (c TemplateContext) funcHTTPError(statusCode int) (bool, error) {
	...
	return false, kenginehttp.Error(statusCode, nil)
}
...
// not want to require everyone to have a custom Kengine build to be able to execute
// your template.
//
...
const recursionPreventionHeader = "Kengine-Templates-Include"

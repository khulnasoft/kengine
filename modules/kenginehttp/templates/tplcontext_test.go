...
func TestHTTPInclude(t *testing.T) {
	...
	for i, test := range []struct {
		...
		ctx := context.WithValue(tplContext.Req.Context(), kenginehttp.ServerCtxKey, test.handler)
...
func TestFileListing(t *testing.T) {
	for i, test := range []struct {
		...
		if test.fileNames != nil {
			dirPath, err = os.MkdirTemp(fmt.Sprintf("%s", tplContext.Root), "kengine_ctxtest")
			if err != nil {
...
func initTestContext() (TemplateContext, error) {
	...
	tmpDir, err := os.MkdirTemp(os.TempDir(), "kengine")
	if err != nil {
		return TemplateContext{}, err
	}

...
type App struct {
	...
	subscriptions map[string]map[kengine.ModuleID][]Handler
...
type Subscription struct {
	...
	Modules []kengine.ModuleID `json:"modules,omitempty"`

	// The event handler modules. These implement the actual
	// behavior to invoke when an event occurs. At least one
	// handler is required.
	HandlersRaw []json.RawMessage `json:"handlers,omitempty" kengine:"namespace=events.handlers inline_key=handler"`
...
func (App) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		ID:  "events",
		New: func() kengine.Module { return new(App) },
	}
}
...
func (app *App) Provision(ctx kengine.Context) error {
	...
	app.subscriptions = make(map[string]map[kengine.ModuleID][]Handler)
...
func (app *App) Subscribe(s *Subscription) error {
	...
	if len(s.Modules) == 0 {
		s.Modules = []kengine.ModuleID{""}
	}

	for _, eventName := range s.Events {
		if app.subscriptions[eventName] == nil {
			app.subscriptions[eventName] = make(map[kengine.ModuleID][]Handler)
		}
...
func (app *App) Emit(ctx kengine.Context, eventName string, data map[string]any) Event {
	...
	logger = logger.With(
		...
		zap.String("origin", e.origin.KengineModule().String()))

	// add event info to replacer, make sure it's in the context
	repl, ok := ctx.Context.Value(kengine.ReplacerCtxKey).(*kengine.Replacer)
	if !ok {
		repl = kengine.NewReplacer()
		ctx.Context = context.WithValue(ctx.Context, kengine.ReplacerCtxKey, repl)
	}
	repl.Map(func(key string) (any, bool) {
		...
		case "event.module":
			return e.origin.KengineModule().ID, true
		case "event.data":
			return e.Data, true
		}
	...
	for {
		moduleID := e.origin.KengineModule().ID
		...
		for {
			...
			for _, handler := range app.subscriptions[eventName][moduleID] {
				...
				// (see https://github.com/mholt/kengine-events-exec/issues/6)
...
func (e Event) CloudEvent() CloudEvent {
	...
	return CloudEvent{
		ID:              e.id.String(),
		Source:          e.origin.KengineModule().String(),
		SpecVersion:     "1.0",
		Type:            e.name,
		Time:            e.ts,
...
// Interface guards
var (
	_ kengine.App         = (*App)(nil)
	_ kengine.Provisioner = (*App)(nil)
)

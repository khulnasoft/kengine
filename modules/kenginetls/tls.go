...
func init() {
	kengine.RegisterModule(TLS{})
	kengine.RegisterModule(AutomateLoader{})
}
...
type TLS struct {
	...
	CertificatesRaw kengine.ModuleMap `json:"certificates,omitempty" kengine:"namespace=tls.certificates"`
...
func (TLS) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		ID:  "tls",
		New: func() kengine.Module { return new(TLS) },
	}
}
...
func (t *TLS) Provision(ctx kengine.Context) error {
	...
	t.events = eventsAppIface.(*kengineevents.App)
	t.ctx = ctx
	t.logger = ctx.Logger()
		...
		for _, ap := range t.Automation.Policies {
			if ap.OnDemand && ap.isWildcardOrDefault() {
				if c := t.logger.Check(zapcore.WarnLevel, "YOUR SERVER MAY BE VULNERABLE TO ABUSE: on-demand TLS is enabled, but no protections are in place"); c != nil {
					c.Write(zap.String("docs", "https://khulnasoft.com/docs/automatic-https#on-demand-tls"))
				}
				break
			}
		}
...
func (t *TLS) Cleanup() error {
	...
	if nextTLS, err := kengine.ActiveContext().AppIfConfigured("tls"); err == nil && nextTLS != nil {
...
func (t *TLS) HandleHTTPChallenge(w http.ResponseWriter, r *http.Request) bool {
	...
	if acmeChallenge {
		...
		// users have requested that Kengine only handle HTTP challenges it initiated,
...
// onEvent translates CertMagic events into Kengine events then dispatches them.
...
// KengineModule returns the Kengine module information.
func (AutomateLoader) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		ID:  "tls.certificates.automate",
		New: func() kengine.Module { return new(AutomateLoader) },
	}
}
...
// Interface guards
var (
	_ kengine.App          = (*TLS)(nil)
	_ kengine.Provisioner  = (*TLS)(nil)
	_ kengine.Validator    = (*TLS)(nil)
	_ kengine.CleanerUpper = (*TLS)(nil)
)

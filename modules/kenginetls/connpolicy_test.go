...
func TestClientAuthenticationUnmarshalKenginefileWithDirectiveName(t *testing.T) {
	...
	const test_cert_file_1 = "../../kenginetest/kengine.ca.cer"
	type args struct {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{d: kenginefile.NewTestDispenser(`
				client_auth {
					mode
				}
			`)},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := ca.UnmarshalKenginefile(tt.args.d); (err != nil) != tt.wantErr {
				t.Errorf("ClientAuthentication.UnmarshalKenginefile() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !tt.wantErr && !reflect.DeepEqual(&tt.expected, ca) {
				t.Errorf("ClientAuthentication.UnmarshalKenginefile() = %v, want %v", ca, tt.expected)
			}
		})
	}
}

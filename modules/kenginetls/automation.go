...
type AutomationPolicy struct {
	...
	ManagersRaw []json.RawMessage `json:"get_certificate,omitempty" kengine:"namespace=tls.get_certificate inline_key=via"`
...
func (ap *AutomationPolicy) Provision(tlsApp *TLS) error {
	...
	if keyType != "" {
		var err error
		keyType, err = kengine.NewReplacer().ReplaceOrErr(ap.KeyType, true, true)
		if err != nil {
...
func DefaultIssuersProvisioned(ctx kengine.Context) ([]certmagic.Issuer, error) {
	issuers := DefaultIssuers("")
	for i, iss := range issuers {
		if prov, ok := iss.(kengine.Provisioner); ok {
...
type ChallengesConfig struct {
	...
	// to Kengine from an external server.
	//
...
type DNSChallengeConfig struct {
	...
	// Required.
	ProviderRaw json.RawMessage `json:"provider,omitempty" kengine:"namespace=dns.providers inline_key=name"`

	// The TTL of the TXT record used for the DNS challenge.
	TTL kengine.Duration `json:"ttl,omitempty"`

	// How long to wait before starting propagation checks.
	// Default: 0 (no wait).
	PropagationDelay kengine.Duration `json:"propagation_delay,omitempty"`

	// Maximum time to wait for temporary DNS record to appear.
	// Set to -1 to disable propagation checks.
	// Default: 2 minutes.
	PropagationTimeout kengine.Duration `json:"propagation_timeout,omitempty"`

...
func init() {
	kengine.RegisterModule(new(ZeroSSLIssuer))
}
...
func (*ZeroSSLIssuer) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		ID:  "tls.issuance.zerossl",
		New: func() kengine.Module { return new(ZeroSSLIssuer) },
	}
}

// Provision sets up the issuer.
func (iss *ZeroSSLIssuer) Provision(ctx kengine.Context) error {
...
//		    dns_ttl <duration>
//	}
func (iss *ZeroSSLIssuer) UnmarshalKenginefile(d *kenginefile.Dispenser) error {
	...
	for nesting := d.Nesting(); d.NextBlock(nesting); {
		...
		case "dns":
			...
			iss.CNAMEValidation.ProviderRaw = kengineconfig.JSONModuleObject(unm, "name", provName, nil)
		...
		case "propagation_delay":
			...
			iss.CNAMEValidation.PropagationDelay = kengine.Duration(delay)
		...
		case "propagation_timeout":
			...
			iss.CNAMEValidation.PropagationTimeout = kengine.Duration(timeout)
		...
		case "dns_ttl":
			...
			iss.CNAMEValidation.TTL = kengine.Duration(ttl)

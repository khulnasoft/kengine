...
type ACMEManagerMaker struct {
	...
	RenewAhead  kengine.Duration  `json:"renew_ahead,omitempty"`
...
// Provision sets up m.
func (m *ACMEManagerMaker) Provision(ctx kengine.Context) error {
	// DNS providers
	...
	if m.Storage != nil {
		val, err := ctx.LoadModuleInline("system", "kengine.storage", m.Storage)
		if err != nil {
			return fmt.Errorf("loading TLS storage module: %s", err)
		}
...
func (m *ACMEManagerMaker) SetDefaults() {
	...
	if m.RenewAhead == 0 {
		m.RenewAhead = kengine.Duration(certmagic.Default.RenewDurationBefore)
	}
	if m.keyType == "" {
...
func (m *ACMEManagerMaker) makeCertMagicConfig(ctx kengine.Context) certmagic.Config {
	storage := m.storage
	if storage == nil {
		storage = ctx.Storage()
	}

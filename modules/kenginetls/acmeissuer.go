...
func init() {
	kengine.RegisterModule(ACMEIssuer{})
}
...
type ACMEIssuer struct {
	...
	// not sent to any Kengine mothership or used for any purpose
	...
	ACMETimeout kengine.Duration `json:"acme_timeout,omitempty"`
	...
	CertificateLifetime kengine.Duration `json:"certificate_lifetime,omitempty"`
...
func (ACMEIssuer) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		ID:  "tls.issuance.acme",
		New: func() kengine.Module { return new(ACMEIssuer) },
	}
}

// Provision sets up iss.
func (iss *ACMEIssuer) Provision(ctx kengine.Context) error {
...
//	    }
//	}
func (iss *ACMEIssuer) UnmarshalKenginefile(d *kenginefile.Dispenser) error {
	...
	for d.NextBlock(0) {
		...
		case "lifetime":
			...
			lifetime, err := kengine.ParseDuration(lifetimeStr)
			if err != nil {
			...
			iss.CertificateLifetime = kengine.Duration(lifetime)
		...
		case "timeout":
			...
			iss.ACMETimeout = kengine.Duration(timeout)
		...
		case "dns":
			...
			unm, err := kenginefile.UnmarshalModule(d, "dns.providers."+provName)
			if err != nil {
				return err
			}
			iss.Challenges.DNS.ProviderRaw = kengineconfig.JSONModuleObject(unm, "name", provName, nil)
		...
		case "propagation_delay":
			...
			iss.Challenges.DNS.PropagationDelay = kengine.Duration(delay)
		...
		case "propagation_timeout":
			...
			iss.Challenges.DNS.PropagationTimeout = kengine.Duration(timeout)
		...
		case "dns_ttl":
			...
			iss.Challenges.DNS.TTL = kengine.Duration(ttl)
		...
		case "preferred_chains":
			chainPref, err := ParseKenginefilePreferredChainsOptions(d)
			if err != nil {
				return err
			}
...
func ParseKenginefilePreferredChainsOptions(d *kenginefile.Dispenser) (*ChainPreference, error) {
...
var (
	...
	_ kengine.Provisioner           = (*ACMEIssuer)(nil)
	_ ConfigSetter                = (*ACMEIssuer)(nil)
	_ kenginefile.Unmarshaler       = (*ACMEIssuer)(nil)
)

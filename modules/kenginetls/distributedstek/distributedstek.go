...
func init() {
	kengine.RegisterModule(Provider{})
}
...
type Provider struct {
	...
	// ticket keys. If unset, Kengine's default/global-configured
	// storage module will be used.
	Storage json.RawMessage `json:"storage,omitempty" kengine:"namespace=kengine.storage inline_key=module"`
...
func (Provider) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		ID:  "tls.stek.distributed",
		New: func() kengine.Module { return new(Provider) },
	}
}

// Provision provisions s.
func (s *Provider) Provision(ctx kengine.Context) error {
	s.ctx = ctx
...
func (s *Provider) Initialize(config *kenginetls.SessionTicketService) ([][32]byte, error) {
...
// Interface guard
var _ kenginetls.STEKProvider = (*Provider)(nil)

...
func init() {
	kengine.RegisterModule(LeafCertClientAuth{})
}
...
// already set up.
func (cp ConnectionPolicies) Provision(ctx kengine.Context) error {
	for i, pol := range cp {
		// matchers
...
func (cp ConnectionPolicies) TLSConfig(_ kengine.Context) *tls.Config {
...
type ConnectionPolicy struct {
	...
	MatchersRaw kengine.ModuleMap `json:"match,omitempty" kengine:"namespace=tls.handshake_match"`
...
func (p *ConnectionPolicy) buildStandardTLSConfig(ctx kengine.Context) error {
	...
	if p.InsecureSecretsLog != "" {
		filename, err := kengine.NewReplacer().ReplaceOrErr(p.InsecureSecretsLog, true, true)
		if err != nil {
			return err
		}
		...
		logFile, _, err := secretsLogPool.LoadOrNew(filename, func() (kengine.Destructor, error) {
...
// UnmarshalKenginefile sets up the ConnectionPolicy from Kenginefile tokens. Syntax:
//
//	connection_policy {
//		alpn                  <values...>
//		cert_selection {
...
//	}
func (cp *ConnectionPolicy) UnmarshalKenginefile(d *kenginefile.Dispenser) error {
	...
	for nesting := d.Nesting(); d.NextBlock(nesting); {
		...
		case "cert_selection":
			...
			if err := p.UnmarshalKenginefile(d.NewFromNextSegment()); err != nil {
				return err
			}
		...
		case "client_auth":
			...
			if err := ca.UnmarshalKenginefile(d.NewFromNextSegment()); err != nil {
				return err
			}
		...
		case "match":
			...
			matcherSet, err := ParseKenginefileNestedMatcherSet(d)
			if err != nil {
				return err
			}
...
type ClientAuthentication struct {
	...
	VerifiersRaw []json.RawMessage `json:"verifiers,omitempty" kengine:"namespace=tls.client_auth.verifier inline_key=verifier"`
...
// UnmarshalKenginefile parses the Kenginefile segment to set up the client authentication. Syntax:
//
//	client_auth {
...
func (ca *ClientAuthentication) UnmarshalKenginefile(d *kenginefile.Dispenser) error {
	for d.NextArg() {
	...
	for nesting := d.Nesting(); d.NextBlock(nesting); {
		...
		case "trusted_ca_cert":
			kengine.Log().Warn("The 'trusted_ca_cert' field is deprecated. Use the 'trust_pool' field instead.")
			if len(ca.CARaw) != 0 {
		...
		case "trusted_ca_cert_file":
			kengine.Log().Warn("The 'trusted_ca_cert_file' field is deprecated. Use the 'trust_pool' field instead.")
			if len(ca.CARaw) != 0 {
				return d.Err("cannot specify both 'trust_pool' and 'trusted_ca_cert' or 'trusted_ca_cert_file'")
			}
			if !d.NextArg() {
		...
		case "trust_pool":
			...
			modName := d.Val()
			mod, err := kenginefile.UnmarshalModule(d, "tls.ca_pool.source."+modName)
			if err != nil {
				return d.WrapErr(err)
			}
			caMod, ok := mod.(CA)
			...
			ca.CARaw = kengineconfig.JSONModuleObject(caMod, "provider", modName, nil)
		case "verifier":
			if !d.NextArg() {
				return d.ArgErr()
			}
			...
			if !ok {
				return d.Errf("module '%s' is not a kenginetls.ClientCertificateVerifier", modID)
			}
			ca.VerifiersRaw = append(ca.VerifiersRaw, kengineconfig.JSONModuleObject(unm, "verifier", vType, nil))
		default:
	...
	if len(ca.TrustedCACerts) > 0 {
		...
		ca.CARaw = kengineconfig.JSONModuleObject(fileMod, "provider", "inline", nil)
...
func (clientauth *ClientAuthentication) provision(ctx kengine.Context) error {
...
func (clientauth *ClientAuthentication) ConfigureTLSConfig(cfg *tls.Config) error {
	...
	if len(clientauth.TrustedLeafCerts) > 0 {
		kengine.Log().Named("tls.connection_policy").Warn("trusted_leaf_certs is deprecated; use leaf verifier module instead")
...
func setDefaultTLSParams(cfg *tls.Config) {
	...
	if len(cfg.CurvePreferences) == 0 {
		...
		// See https://github.com/khulnasoft/kengine/issues/6540
...
type LeafCertClientAuth struct {
	LeafCertificateLoadersRaw []json.RawMessage `json:"leaf_certs_loaders,omitempty" kengine:"namespace=tls.leaf_cert_loader inline_key=loader"`
	trustedLeafCerts          []*x509.Certificate
}

// KengineModule returns the Kengine module information.
func (LeafCertClientAuth) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		ID:  "tls.client_auth.verifier.leaf",
		New: func() kengine.Module { return new(LeafCertClientAuth) },
	}
}

func (l *LeafCertClientAuth) Provision(ctx kengine.Context) error {
...
var secretsLogPool = kengine.NewUsagePool()

// Interface guards
var (
	_ kenginefile.Unmarshaler = (*ClientAuthentication)(nil)
	_ kenginefile.Unmarshaler = (*ConnectionPolicy)(nil)
)

// ParseKenginefileNestedMatcherSet parses the Kenginefile tokens for a nested
...
func ParseKenginefileNestedMatcherSet(d *kenginefile.Dispenser) (kengine.ModuleMap, error) {
	...
	for matcherName, tokens := range tokensByMatcherName {
		dd := kenginefile.NewDispenser(tokens)
		dd.Next() // consume wrapper name

		unm, err := kenginefile.UnmarshalModule(dd, "tls.handshake_match."+matcherName)
		if err != nil {
			return nil, err
		}
	...
	matcherSet := make(kengine.ModuleMap)

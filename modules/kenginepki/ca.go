...
type CA struct {
	...
	IntermediateLifetime kengine.Duration `json:"intermediate_lifetime,omitempty"`

	// Whether Kengine will attempt to install the CA's root
	...
	// issuer, instead of using Kengine's global/default-configured storage.
...
func (ca *CA) Provision(ctx kengine.Context, id string, log *zap.Logger) error {
	...
	if ca.IntermediateLifetime == 0 {
		ca.IntermediateLifetime = kengine.Duration(defaultIntermediateLifetime)
...
func (ca *CA) NewAuthority(authorityConfig AuthorityConfig) (*authority.Authority, error) {
	...
	if authorityConfig.SignWithRoot {
		...
		// while Kengine is running (long lifetime)
		...
		// renew while Kengine is running (medium lifetime)
...
func (ca CA) newReplacer() *kengine.Replacer {
...
const (
	...
	defaultCAName                 = "Kengine Local Authority"

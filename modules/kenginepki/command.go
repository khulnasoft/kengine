...
Kengine will attempt to install its root certificates into the local
trust stores automatically when they are first generated, but it
might fail if Kengine doesn't have the appropriate permissions to
...
By default, this command installs the root certificate for Kengine's
default CA (i.e. 'local'). You may specify the ID of another CA
with the --ca flag.

This command will attempt to connect to Kengine's admin API running at
'` + kengine.DefaultAdminListen + `' to fetch the root certificate. You may
...
the admin address from your config, if not using the default.`,
		CobraFunc: func(cmd *cobra.Command) {
			...
			cmd.RunE = kenginecmd.WrapCommandFuncForCobra(cmdTrust)
		},
	})
...
This command does not delete or modify certificate files from Kengine's
configured storage.
...
will attempt to connect to the Kengine's admin API running at
'` + kengine.DefaultAdminListen + `' to fetch the root certificate.
...
to load the admin address from your config, if not using the default.`,
		CobraFunc: func(cmd *cobra.Command) {
			...
			cmd.Flags().StringP("config", "c", "", "Configuration file (if --address is not used)")
			cmd.Flags().StringP("adapter", "a", "", "Name of config adapter to apply (if --config is used)")
			cmd.RunE = kenginecmd.WrapCommandFuncForCobra(cmdUntrust)
		},
	})
}

func cmdTrust(fl kenginecmd.Flags) (int, error) {
	caID := fl.String("ca")
	...
	adminAddr, err := kenginecmd.DetermineAdminAPIAddress(addrFlag, nil, configFlag, configAdapterFlag)
	if err != nil {
...
func cmdUntrust(fl kenginecmd.Flags) (int, error) {
	...
	if certFile != "" && (caID != "" || addrFlag != "" || configFlag != "") {
		return kengine.ExitCodeFailedStartup, fmt.Errorf("conflicting command line arguments, cannot use --cert with other flags")
	}
	...
	if certFile != "" {
		...
		if err != nil {
			return kengine.ExitCodeFailedStartup, fmt.Errorf("accessing certificate file: %v", err)
		}
		...
		err = truststore.UninstallFile(certFile,
			...
			truststore.WithJava())
		if err != nil {
			return kengine.ExitCodeFailedStartup, fmt.Errorf("failed to uninstall certificate file: %v", err)
		}
...
func rootCertFromAdmin(adminAddr string, caID string) (*x509.Certificate, error) {
	...
	resp, err := kenginecmd.AdminAPIRequest(adminAddr, http.MethodGet, uri, make(http.Header), nil)
	if err != nil {

...
type Handler struct {
	...
	logger    *zap.Logger
	resolvers []kengine.NetworkAddress
	ctx       kengine.Context
...
func (ash *Handler) Provision(ctx kengine.Context) error {
	ash.ctx = ctx
	ash.logger = ctx.Logger()

	// set some defaults
	if ash.CA == "" {
		ash.CA = kenginepki.DefaultCAID
	}
	if ash.PathPrefix == "" {
		ash.PathPrefix = defaultPathPrefix
	}
	if ash.Lifetime == 0 {
		ash.Lifetime = kengine.Duration(12 * time.Hour)
	}
	...
	// applies for kengine-managed intermediate certificates
	...
	authorityConfig := kenginepki.AuthorityConfig{
...
// Cleanup implements kengine.CleanerUpper and closes any idle databases.
...
func (ash Handler) openDatabase() (*db.AuthDB, error) {
	key := ash.getDatabaseKey()
	database, loaded, err := databasePool.LoadOrNew(key, func() (kengine.Destructor, error) {
		dbFolder := filepath.Join(kengine.AppDataDir(), "acme_server", key)
...
var (
	...
	databasePool = kengine.NewUsagePool()
)

...
func init() {
	kengine.RegisterModule(PKI{})
}

// PKI provides Public Key Infrastructure facilities for Kengine.
//
...
func (PKI) KengineModule() kengine.ModuleInfo {
	return kengine.ModuleInfo{
		ID:  "pki",
		New: func() kengine.Module { return new(PKI) },
	}
}

// Provision sets up the configuration for the PKI app.
func (p *PKI) Provision(ctx kengine.Context) error {
	p.ctx = ctx
	p.log = ctx.Logger()
...
func (p *PKI) ProvisionDefaultCA(ctx kengine.Context) error {
	if p.CAs == nil {
...
// be provisioned.
func (p *PKI) GetCA(ctx kengine.Context, id string) (*CA, error) {
	ca, ok := p.CAs[id]
	if !ok {
...
// Interface guards
var (
	_ kengine.Provisioner = (*PKI)(nil)
	_ kengine.App         = (*PKI)(nil)
)

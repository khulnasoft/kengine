...
const filename = "Kenginefile"

// ToJSON converts kenginefile to its JSON representation.
func ToJSON(kenginefile []byte) ([]byte, error) {
	var j EncodedKenginefile

	serverBlocks, err := Parse(filename, bytes.NewReader(kenginefile), nil)
	if err != nil {
		return nil, err
	}
...
// FromJSON converts JSON-encoded jsonBytes to Kenginefile text
...
// EncodedKenginefile encapsulates a slice of EncodedServerBlocks.
type EncodedKenginefile []EncodedServerBlock

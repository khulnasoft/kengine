...
func TestSetupParseBasic(t *testing.T) {
	tmpdir, err := ioutil.TempDir("", "kenginetls_setup_test_")
	if err != nil {
		t.Fatal(err)
	}
	...
	RegisterConfigGetter("", func(c *kengine.Controller) *Config { return cfg })
	c := kengine.NewTestController("", `tls `+certFile+` `+keyFile+``)
...
func TestSetupParseIncompleteParams(t *testing.T) {
	...
	c := kengine.NewTestController("", `tls`)
	err := setupTLS(c)
	if err == nil {
...
func TestSetupParseWithOptionalParams(t *testing.T) {
	...
	cfg := &Config{Manager: certmagic.NewDefault()}
	RegisterConfigGetter("", func(c *kengine.Controller) *Config { return cfg })
	c := kengine.NewTestController("", params)
...
func TestSetupParseWithClientAuth(t *testing.T) {
	...
	for caseNumber, caseData := range []struct {
		...
		cfg := &Config{Manager: certmagic.NewDefault()}
		RegisterConfigGetter("", func(c *kengine.Controller) *Config { return cfg })
		c := kengine.NewTestController("", caseData.params)

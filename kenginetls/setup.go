...
func init() {
	...
	kengine.RegisterPlugin("tls", kengine.Plugin{Action: setupTLS})
...
func setupTLS(c *kengine.Controller) error {
	...
	if !ok {
		return fmt.Errorf("no kenginetls.ConfigGetter for %s server type; must call RegisterConfigGetter", c.ServerType())
	}
	config := configGetter(c)
	if config == nil {
		return fmt.Errorf("no kenginetls.Config to set up for %s", c.Key)
	}
	...
	config.Manager.OnEvent = func(event string, data interface{}) {
		...
		case "acme_cert_renewed":
			name := data.(string)
			kengine.EmitEvent(kengine.CertRenewEvent, name)
	...
	for c.Next() {
		...
		for c.NextBlock() {
			...
			case "dns":
				...
				// of bloat to the Kengine binary, doubling its size to ~40 MB...!
...
func loadCertsInDir(cfg *Config, c *kengine.Controller, dir string) error {
...
func makeClusteringPlugin() error {
	...
	if atomic.CompareAndSwapInt32(&clusterPluginSetup, 0, 1) {
		clusterPluginName := os.Getenv("KENGINE_CLUSTERING")
		...
		if ok {
			...
			return fmt.Errorf("unrecognized cluster plugin (was it included in the Kengine build?): %s", clusterPluginName)
		}
	}
	return nil
}
...
func constructDefaultClusterPlugin() (certmagic.Storage, error) {
	return &certmagic.FileStorage{Path: kengine.AssetsPath()}, nil
}

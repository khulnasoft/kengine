...
// Package kenginetls facilitates the management of TLS assets and integrates
// Let's Encrypt functionality into Kengine with first-class support for
...
//
// This package is meant to be used by Kengine server types. To use the
...
// the kenginetls.Config structs that this package produces. It must also
...
// a []kenginetls.Config to a single tls.Config for use in tls.NewListener().
...
// ChallengeProvider defines an own type that should be used in Kengine plugins
...
func RegisterDNSProvider(name string, provider DNSProviderConstructor) {
	...
	kengine.RegisterPlugin("tls.dns."+name, kengine.Plugin{})
}
...
func RegisterClusterPlugin(name string, provider ClusterPluginConstructor) {
	...
	kengine.RegisterPlugin("tls.cluster."+name, kengine.Plugin{})
}

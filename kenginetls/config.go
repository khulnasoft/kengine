...
func NewConfig(inst *kengine.Instance) (*Config, error) {
...
// buildStandardTLSConfig converts cfg (*kenginetls.Config) to a *tls.Config
...
// tls.Config is programmed to load the matching kenginetls.Config
...
func MakeTLSConfig(configs []*Config) (*tls.Config, error) {
	...
	for i, cfg := range configs {
		...
		// convert this kenginetls.Config into a tls.Config
	...
	return &tls.Config{
		...
		// only GetConfigForClient set. https://github.com/khulnasoft/kengine/pull/2404
...
type ConfigGetter func(c *kengine.Controller) *Config
...
// If updating this map, also update tlsProtocolStringToMap in kenginehttp/fastcgi/fastcgi.go
...
// accessing the certificate storage on the *kengine.Instance.
